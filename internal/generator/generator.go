...
func GenerateBaseFields(conf CurveConfig, outputDir string) error {
	if err := goff.GenerateFF("fr", "Element", conf.RTorsion, filepath.Join(outputDir, "fr"), false); err != nil {
		return err
	}
	if err := goff.GenerateFF("fp", "Element", conf.FpModulus, filepath.Join(outputDir, "fp"), false); err != nil {
		return err
	}
	return nil
}
...
func GenerateFq12over6over2(conf CurveConfig, outputDir string) error {
...

	{
		...
		if err := goff.GenerateFF2(conf.CurveName, "E2", conf.FpModulus, outputDir); err != nil {
			return err
		}
	}

	{
		...
		pathSrc := filepath.Join(outputDir, "e2_fallback.go")

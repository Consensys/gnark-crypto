...
func generateFFT(F *config.Field, fft *config.FFT, outputDir string) error {
	...
	if data.HasASMKernel {
		...
		// generate the AMD64 assembly file
		...
		entries = append(entries,
			bavard.Entry{
				File:      filepath.Join(outputDir, "kernel_arm64.go"),
				Templates: []string{"kernel.arm64.go.tmpl"},
				BuildTag:  "!purego"})

		// generate the ARM64 assembly file
		fftKernelsArm64, err := os.Create(filepath.Join(outputDir, "kernel_arm64.s"))
		if err != nil {
			return err
		}

		fftKernelsArm64.WriteString("//go:build !purego\n\n")
		fftKernelsArm64.WriteString("// Code generated by gnark-crypto/generator. DO NOT EDIT.\n\n")
		fftKernelsArm64.WriteString("#include \"textflag.h\"\n")
		fftKernelsArm64.WriteString("#include \"funcdata.h\"\n")
		fftKernelsArm64.WriteString("#include \"go_asm.h\"\n\n")

		fArm64 := arm64.NewFFArm64(fftKernelsArm64, 1)
		if err := arm64.GenerateF31FFTKernels(fArm64, data.Kernels); err != nil {
			fftKernelsArm64.Close()
			return err
		}
		fftKernelsArm64.Close()
	}
...
type fftTemplateData struct {
	...
	Q, QInvNeg       uint64
	Mu               uint64 // mu = 2^64 / q for ARM64 Montgomery reduction
	F31              bool
}

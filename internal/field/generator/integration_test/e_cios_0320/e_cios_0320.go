...
// Modulus q =
//
//	q[base10] = 1828330821037753172724466375214676479989751853919331159872306617334304448959201039278848606261431
//	q[base16] = 0xdb208b047d1eb1159c18b39c3edb3c996145150390f85da6b3f2bc53f2911d19a459f339c140d0b7
//
// # Warning
//
...
const (
	Limbs = 5         // number of 64 bits words needed to represent a e_cios_0320
	Bits  = 320       // number of bits needed to represent a e_cios_0320
	Bytes = Limbs * 8 // number of bytes needed to represent a e_cios_0320
)

// Field modulus q
const (
	q0 uint64 = 11842764124528365751
	q1 uint64 = 12966633345890983193
	q2 uint64 = 7009031500133326246
	q3 uint64 = 11247937553008639129
	q4 uint64 = 15789773144956252437
)
...
const qInvNeg uint64 = 15752526571658343161
...
func init() {
	_modulus.SetString("db208b047d1eb1159c18b39c3edb3c996145150390f85da6b3f2bc53f2911d19a459f339c140d0b7", 16)
}

// Newe_cios_0320 returns a new e_cios_0320 from a uint64 value
//
// it is equivalent to
//
//	var v e_cios_0320
//	v.SetUint64(...)
func Newe_cios_0320(v uint64) e_cios_0320 {
	z := e_cios_0320{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_cios_0320) SetInt64(v int64) *e_cios_0320 {
...
func (z *e_cios_0320) SetInterface(i1 interface{}) (*e_cios_0320, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_cios_0320 with <nil>")
	}
...
func (z *e_cios_0320) SetOne() *e_cios_0320 {
	z[0] = 6603979949181185865
	z[1] = 5480110727818568422
	z[2] = 11437712573576225369
	z[3] = 7198806520700912486
	z[4] = 2656970928753299178
	return z
}
...
func (z *e_cios_0320) NotEqual(x *e_cios_0320) uint64 {
...
// IsOne returns z == 1
func (z *e_cios_0320) IsOne() bool {
	return (z[4] ^ 2656970928753299178 | z[3] ^ 7198806520700912486 | z[2] ^ 11437712573576225369 | z[1] ^ 5480110727818568422 | z[0] ^ 6603979949181185865) == 0
}
...
func (z *e_cios_0320) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_cios_0320) FitsOnOneWord() bool {
...
func (z *e_cios_0320) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 15144754099118958684, 0)
	_, b = bits.Sub64(_z[1], 6483316672945491596, b)
	_, b = bits.Sub64(_z[2], 12727887786921438931, b)
	_, b = bits.Sub64(_z[3], 14847340813359095372, b)
	_, b = bits.Sub64(_z[4], 7894886572478126218, b)
...
func (z *e_cios_0320) smallerThanModulus() bool {
...
func (z *e_cios_0320) Select(c int, x0 *e_cios_0320, x1 *e_cios_0320) *e_cios_0320 {
...
// Exp z = xᵏ (mod q)
func (z *e_cios_0320) Exp(x e_cios_0320, k *big.Int) *e_cios_0320 {
...
func (z *e_cios_0320) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_cios_0320) SetString(number string) (*e_cios_0320, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_cios_0320.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_cios_0320) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_cios_0320.SetString for valid prefixes (0x, 0b, ...)
func (z *e_cios_0320) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_cios_0320.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_cios_0320, _ = new(big.Int).SetString("6d9045823e8f588ace0c59ce1f6d9e4cb0a28a81c87c2ed359f95e29f9488e8cd22cf99ce0a0685b", 16)
	const sqrtExponente_cios_0320 = "36c822c11f47ac4567062ce70fb6cf2658514540e43e1769acfcaf14fca4474669167cce7050342e"

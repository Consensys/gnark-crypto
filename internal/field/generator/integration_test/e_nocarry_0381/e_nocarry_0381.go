...
// Modulus q =
//
//	q[base10] = 4730824005872000375807198005519171430939361304000509079974608596955745080768107645986063361890707709362136541727141
//	q[base16] = 0x1ebc9ddcdae2ff714d8ae409b4d8c641a5dd7cb04cc30a1fc9a48f30cfcbc001e96ebedfa470f4b50d20fb5bb81a89a5
//
// # Warning
//
...
const (
	Limbs = 6         // number of 64 bits words needed to represent a e_nocarry_0381
	Bits  = 381       // number of bits needed to represent a e_nocarry_0381
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0381
)

// Field modulus q
const (
	q0 uint64 = 946032293097146789
	q1 uint64 = 16820591526021231797
	q2 uint64 = 14529895737611501569
	q3 uint64 = 11951846082755103263
	q4 uint64 = 5587529018046465601
	q5 uint64 = 2214818688650116977
)
...
const qInvNeg uint64 = 8244443496882555347
...
func init() {
	_modulus.SetString("1ebc9ddcdae2ff714d8ae409b4d8c641a5dd7cb04cc30a1fc9a48f30cfcbc001e96ebedfa470f4b50d20fb5bb81a89a5", 16)
}

// Newe_nocarry_0381 returns a new e_nocarry_0381 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0381
//	v.SetUint64(...)
func Newe_nocarry_0381(v uint64) e_nocarry_0381 {
	z := e_nocarry_0381{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0381) SetInt64(v int64) *e_nocarry_0381 {
...
func (z *e_nocarry_0381) SetInterface(i1 interface{}) (*e_nocarry_0381, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0381 with <nil>")
	}
...
func (z *e_nocarry_0381) SetOne() *e_nocarry_0381 {
	z[0] = 10878485728932377304
	z[1] = 13009220381506558551
	z[2] = 12888042615074848752
	z[3] = 15065695780216483585
	z[4] = 10640000076756930034
	z[5] = 728194564508615797
	return z
}
...
func (z *e_nocarry_0381) NotEqual(x *e_nocarry_0381) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0381) IsOne() bool {
	return (z[5] ^ 728194564508615797 | z[4] ^ 10640000076756930034 | z[3] ^ 15065695780216483585 | z[2] ^ 12888042615074848752 | z[1] ^ 13009220381506558551 | z[0] ^ 10878485728932377304) == 0
}
...
func (z *e_nocarry_0381) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0381) FitsOnOneWord() bool {
...
func (z *e_nocarry_0381) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 9696388183403349203, 0)
	_, b = bits.Sub64(_z[1], 17633667799865391706, b)
	_, b = bits.Sub64(_z[2], 16488319905660526592, b)
	_, b = bits.Sub64(_z[3], 15199295078232327439, b)
	_, b = bits.Sub64(_z[4], 12017136545878008608, b)
	_, b = bits.Sub64(_z[5], 1107409344325058488, b)
...
func (z *e_nocarry_0381) smallerThanModulus() bool {
...
func (z *e_nocarry_0381) Select(c int, x0 *e_nocarry_0381, x1 *e_nocarry_0381) *e_nocarry_0381 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0381) Exp(x e_nocarry_0381, k *big.Int) *e_nocarry_0381 {
...
func (z *e_nocarry_0381) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0381) SetString(number string) (*e_nocarry_0381, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0381.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0381) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0381.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0381) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0381.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0381, _ = new(big.Int).SetString("f5e4eee6d717fb8a6c57204da6c6320d2eebe582661850fe4d2479867e5e000f4b75f6fd2387a5a86907daddc0d44d2", 16)
	const sqrtExponente_nocarry_0381 = "3d793bb9b5c5fee29b15c81369b18c834bbaf9609986143f93491e619f978003d2dd7dbf48e1e96a1a41f6b77035134"
...
func (z *e_nocarry_0381) Sqrt(x *e_nocarry_0381) *e_nocarry_0381 {
	...
	var one, alpha, beta, tx, square e_nocarry_0381
	one.SetOne()
	tx.Double(x)
	alpha.Exp(tx, _bSqrtExponente_nocarry_0381)
	beta.Square(&alpha).
		Mul(&beta, &tx).
		Sub(&beta, &one).
		Mul(&beta, x).
...
const (
	inversionCorrectionFactorWord0 = 12278769188608335254
	inversionCorrectionFactorWord1 = 18429262015527255046
	inversionCorrectionFactorWord2 = 6181514433162109957
	inversionCorrectionFactorWord3 = 17177426884959562016
	inversionCorrectionFactorWord4 = 13341092422399416156
	inversionCorrectionFactorWord5 = 1897637334492246874
	invIterationsN                 = 26
)
...
func (z *e_nocarry_0381) inverseExp(x *e_nocarry_0381) *e_nocarry_0381 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0381, nBits int) uint64 {
...
// 0 ≤ x, y < 2³⁸¹
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0381) linearComb(x *e_nocarry_0381, xC int64, y *e_nocarry_0381, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³⁸¹ = 2⁴⁴⁵
	// therefore | hi | < 2⁶¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0381, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0381) mulWNonModular(x *e_nocarry_0381, y int64) uint64 {

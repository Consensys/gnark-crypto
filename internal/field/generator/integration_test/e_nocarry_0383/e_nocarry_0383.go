...
const (
	Limbs = 6         // number of 64 bits words needed to represent a e_nocarry_0383
	Bits  = 383       // number of bits needed to represent a e_nocarry_0383
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0383
)

// Field modulus q
const (
	q0 uint64 = 17849363352279540679
	q1 uint64 = 5695776312768086757
	q2 uint64 = 3746241199866308157
	q3 uint64 = 6369639522295119120
	q4 uint64 = 17352731420369788280
	q5 uint64 = 8381515075543308361
)
...
const qInvNeg uint64 = 6885517277990654473
...
func init() {
	_modulus.SetString("74511f8dd5571849f0d14949c1e69d7858658165d1b07d1033fd5285d8f6e63d4f0b766102a042e5f7b5ad661f7f63c7", 16)
}

// Newe_nocarry_0383 returns a new e_nocarry_0383 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0383
//	v.SetUint64(...)
func Newe_nocarry_0383(v uint64) e_nocarry_0383 {
	z := e_nocarry_0383{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0383) SetInt64(v int64) *e_nocarry_0383 {
...
func (z *e_nocarry_0383) SetInterface(i1 interface{}) (*e_nocarry_0383, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0383 with <nil>")
	}
...
func (z *e_nocarry_0383) SetOne() *e_nocarry_0383 {
	z[0] = 1194761442860021874
	z[1] = 7055191448173378100
	z[2] = 10954261673976935301
	z[3] = 5707465029119313375
	z[4] = 2188025306679526671
	z[5] = 1683713922622934892
	return z
}
...
func (z *e_nocarry_0383) NotEqual(x *e_nocarry_0383) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0383) IsOne() bool {
	return (z[5] ^ 1683713922622934892 | z[4] ^ 2188025306679526671 | z[3] ^ 5707465029119313375 | z[2] ^ 10954261673976935301 | z[1] ^ 7055191448173378100 | z[0] ^ 1194761442860021874) == 0
}
...
func (z *e_nocarry_0383) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0383) FitsOnOneWord() bool {
...
func (z *e_nocarry_0383) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 18148053712994546148, 0)
	_, b = bits.Sub64(_z[1], 12071260193238819186, b)
	_, b = bits.Sub64(_z[2], 1873120599933154078, b)
	_, b = bits.Sub64(_z[3], 3184819761147559560, b)
	_, b = bits.Sub64(_z[4], 17899737747039669948, b)
	_, b = bits.Sub64(_z[5], 4190757537771654180, b)
...
func (z *e_nocarry_0383) smallerThanModulus() bool {
...
func (z *e_nocarry_0383) Select(c int, x0 *e_nocarry_0383, x1 *e_nocarry_0383) *e_nocarry_0383 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0383) Exp(x e_nocarry_0383, k *big.Int) *e_nocarry_0383 {
...
func (z *e_nocarry_0383) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0383) SetString(number string) (*e_nocarry_0383, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0383.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0383) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0383.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0383) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0383.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0383, _ = new(big.Int).SetString("3a288fc6eaab8c24f868a4a4e0f34ebc2c32c0b2e8d83e8819fea942ec7b731ea785bb3081502172fbdad6b30fbfb1e3", 16)
	const sqrtExponente_nocarry_0383 = "1d1447e37555c6127c3452527079a75e16196059746c1f440cff54a1763db98f53c2dd9840a810b97ded6b5987dfd8f2"
...
func (z *e_nocarry_0383) Sqrt(x *e_nocarry_0383) *e_nocarry_0383 {
	...
	var y, square e_nocarry_0383
	y.Exp(*x, _bSqrtExponente_nocarry_0383)
...
const (
	inversionCorrectionFactorWord0 = 9037800158364755045
	inversionCorrectionFactorWord1 = 17687550359706345234
	inversionCorrectionFactorWord2 = 7737772917986068233
	inversionCorrectionFactorWord3 = 6075387127166715710
	inversionCorrectionFactorWord4 = 12397577025192628638
	inversionCorrectionFactorWord5 = 6744943884904342128
	invIterationsN                 = 26
)
...
func (z *e_nocarry_0383) inverseExp(x *e_nocarry_0383) *e_nocarry_0383 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0383, nBits int) uint64 {
...
// 0 ≤ x, y < 2³⁸³
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0383) linearComb(x *e_nocarry_0383, xC int64, y *e_nocarry_0383, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³⁸³ = 2⁴⁴⁷
	// therefore | hi | < 2⁶³ ≤ 2⁶³
...
func negL(x *e_nocarry_0383, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0383) mulWNonModular(x *e_nocarry_0383, y int64) uint64 {

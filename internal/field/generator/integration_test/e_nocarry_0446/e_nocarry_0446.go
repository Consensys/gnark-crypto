...
const (
	Limbs = 7         // number of 64 bits words needed to represent a e_nocarry_0446
	Bits  = 446       // number of bits needed to represent a e_nocarry_0446
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0446
)

// Field modulus q
const (
	q0 uint64 = 14653028473948445665
	q1 uint64 = 2751839678416873866
	q2 uint64 = 4827050415832752706
	q3 uint64 = 9955135279234597082
	q4 uint64 = 9938285786482498484
	q5 uint64 = 40516769746661530
	q6 uint64 = 3862264439513386515
)
...
const qInvNeg uint64 = 3552927375576012767
...
func init() {
	_modulus.SetString("3599850b69d18e13008ff1c99432309a89ebe2447b9a5bb48a27becab9bcc0da42fd20c7449ce24226307f9b8edce98acb5a03c311619be1", 16)
}

// Newe_nocarry_0446 returns a new e_nocarry_0446 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0446
//	v.SetUint64(...)
func Newe_nocarry_0446(v uint64) e_nocarry_0446 {
	z := e_nocarry_0446{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0446) SetInt64(v int64) *e_nocarry_0446 {
...
func (z *e_nocarry_0446) SetInterface(i1 interface{}) (*e_nocarry_0446, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0446 with <nil>")
	}
...
func (z *e_nocarry_0446) SetOne() *e_nocarry_0446 {
	z[0] = 15174862399044423804
	z[1] = 7439385360042056148
	z[2] = 17585286484088092407
	z[3] = 15519691104190266518
	z[4] = 15587089075198660909
	z[5] = 18284676994722905493
	z[6] = 2997686315656005555
	return z
}
...
func (z *e_nocarry_0446) NotEqual(x *e_nocarry_0446) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0446) IsOne() bool {
	return (z[6] ^ 2997686315656005555 | z[5] ^ 18284676994722905493 | z[4] ^ 15587089075198660909 | z[3] ^ 15519691104190266518 | z[2] ^ 17585286484088092407 | z[1] ^ 7439385360042056148 | z[0] ^ 15174862399044423804) == 0
}
...
func (z *e_nocarry_0446) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0446) FitsOnOneWord() bool {
...
func (z *e_nocarry_0446) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 7326514236974222833, 0)
	_, b = bits.Sub64(_z[1], 1375919839208436933, b)
	_, b = bits.Sub64(_z[2], 2413525207916376353, b)
	_, b = bits.Sub64(_z[3], 4977567639617298541, b)
	_, b = bits.Sub64(_z[4], 4969142893241249242, b)
	_, b = bits.Sub64(_z[5], 9243630421728106573, b)
	_, b = bits.Sub64(_z[6], 1931132219756693257, b)
...
func (z *e_nocarry_0446) smallerThanModulus() bool {
...
func (z *e_nocarry_0446) Select(c int, x0 *e_nocarry_0446, x1 *e_nocarry_0446) *e_nocarry_0446 {
...
func _mulGeneric(z, x, y *e_nocarry_0446) {
	...
	var c [3]uint64
	{
		...
		t[6], t[5] = madd3(m, q6, c[0], c[2], c[1])
	}
	{
		// round 1
		v := x[1]
	...
	}
	{
		...
		c[1], c[0] = madd2(v, y[6], c[1], t[6])
		z[6], z[5] = madd3(m, q6, c[0], c[2], c[1])
	}
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0446) Exp(x e_nocarry_0446, k *big.Int) *e_nocarry_0446 {
...
var rSquare = e_nocarry_0446{
	9054857777615806593,
	6225932862557331550,
	13127043735039903208,
	267306239436653364,
	12704056256933310309,
	7176233673543078269,
	3715481722695674058,
}
...
func (z *e_nocarry_0446) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0446) SetString(number string) (*e_nocarry_0446, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0446.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0446) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0446.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0446) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0446.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0446, _ = new(big.Int).SetString("1accc285b4e8c7098047f8e4ca19184d44f5f1223dcd2dda4513df655cde606d217e9063a24e712113183fcdc76e74c565ad01e188b0cdf0", 16)
	const sqrtExponente_nocarry_0446 = "d666142da746384c023fc72650c8c26a27af8911ee696ed2289efb2ae6f303690bf4831d1273890898c1fe6e3b73a62b2d680f0c45866f"
...
func (z *e_nocarry_0446) Sqrt(x *e_nocarry_0446) *e_nocarry_0446 {
	...
	var y, b, t, w e_nocarry_0446
	// w = x^((s-1)/2))
	w.Exp(*x, _bSqrtExponente_nocarry_0446)
	...
	// g = nonResidue ^ s
	var g = e_nocarry_0446{
		6078254751513721550,
		1150968097147711562,
		12518325894301875990,
		5622133350736857845,
		11931074693582514153,
		9518557482038626740,
		3410914730766473144,
	}
	r := uint64(5)
...
const (
	inversionCorrectionFactorWord0 = 10852045619230508995
	inversionCorrectionFactorWord1 = 13923819517007488813
	inversionCorrectionFactorWord2 = 11173010792966269117
	inversionCorrectionFactorWord3 = 11069563995911862788
	inversionCorrectionFactorWord4 = 13654648599983299808
	inversionCorrectionFactorWord5 = 2260537918443905707
	inversionCorrectionFactorWord6 = 3165998256216569998
	invIterationsN                 = 30
)
...
func (z *e_nocarry_0446) inverseExp(x *e_nocarry_0446) *e_nocarry_0446 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0446, nBits int) uint64 {
...
// 0 ≤ x, y < 2⁴⁴⁶
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0446) linearComb(x *e_nocarry_0446, xC int64, y *e_nocarry_0446, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2⁴⁴⁶ = 2⁵¹⁰
	// therefore | hi | < 2⁶² ≤ 2⁶³
...
func (z *e_nocarry_0446) montReduceSigned(x *e_nocarry_0446, xHi uint64) {
	...
	// m * qElement[6] ≤ (2⁶⁴ - 1) * (2⁶³ - 1) = 2¹²⁷ - 2⁶⁴ - 2⁶³ + 1
	// x[6] + C ≤ 2*(2⁶⁴ - 1) = 2⁶⁵ - 2
	// On LHS, (C, t[6]) ≤ 2¹²⁷ - 2⁶⁴ - 2⁶³ + 1 + 2⁶⁵ - 2 = 2¹²⁷ + 2⁶³ - 1
	// So on LHS, C ≤ 2⁶³
	t[7] = xHi + C
	// xHi + C < 2⁶³ + 2⁶³ = 2⁶⁴
	...
	if mustNeg {
		...
		if b != 0 {
			...
			z[4], carry = bits.Add64(z[4], q4, carry)
			z[5], carry = bits.Add64(z[5], q5, carry)
			z[6], _ = bits.Add64(neg1, q6, carry)
		}
	}
}
...
func negL(x *e_nocarry_0446, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0446) mulWNonModular(x *e_nocarry_0446, y int64) uint64 {

...
// Modulus q =
//
//	q[base10] = 197365488285857068843306506540423895936536808428608290728920383454412855126349
//	q[base16] = 0x1b458e85ed5515998ae9997fb4bd48881d27b853964b8f2a43da2a89eca29294d
//
// # Warning
//
...
const (
	Limbs = 5         // number of 64 bits words needed to represent a e_nocarry_0257
	Bits  = 257       // number of bits needed to represent a e_nocarry_0257
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0257
)

// Field modulus q
const (
	q0 uint64 = 4441297582490724685
	q1 uint64 = 15166862651649749668
	q2 uint64 = 12581254139600996481
	q3 uint64 = 12995392218781014424
	q4 uint64 = 1
)
...
const qInvNeg uint64 = 12598831468807571579
...
func init() {
	_modulus.SetString("1b458e85ed5515998ae9997fb4bd48881d27b853964b8f2a43da2a89eca29294d", 16)
}

// Newe_nocarry_0257 returns a new e_nocarry_0257 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0257
//	v.SetUint64(...)
func Newe_nocarry_0257(v uint64) e_nocarry_0257 {
	z := e_nocarry_0257{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0257) SetInt64(v int64) *e_nocarry_0257 {
...
func (z *e_nocarry_0257) SetInterface(i1 interface{}) (*e_nocarry_0257, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0257 with <nil>")
	}
...
func (z *e_nocarry_0257) SetOne() *e_nocarry_0257 {
	z[0] = 15235810490944928340
	z[1] = 4354056100861905068
	z[2] = 10914620305581525621
	z[3] = 1978729230039523454
	z[4] = 1
	return z
}
...
func (z *e_nocarry_0257) NotEqual(x *e_nocarry_0257) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0257) IsOne() bool {
	return (z[4] ^ 1 | z[3] ^ 1978729230039523454 | z[2] ^ 10914620305581525621 | z[1] ^ 4354056100861905068 | z[0] ^ 15235810490944928340) == 0
}
...
func (z *e_nocarry_0257) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0257) FitsOnOneWord() bool {
...
func (z *e_nocarry_0257) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 2220648791245362343, 0)
	_, b = bits.Sub64(_z[1], 16806803362679650642, b)
	_, b = bits.Sub64(_z[2], 6290627069800498240, b)
	_, b = bits.Sub64(_z[3], 15721068146245283020, b)
...
func (z *e_nocarry_0257) smallerThanModulus() bool {
...
func (z *e_nocarry_0257) Select(c int, x0 *e_nocarry_0257, x1 *e_nocarry_0257) *e_nocarry_0257 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0257) Exp(x e_nocarry_0257, k *big.Int) *e_nocarry_0257 {
...
func (z *e_nocarry_0257) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0257) SetString(number string) (*e_nocarry_0257, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0257.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0257) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0257.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0257) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0257.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0257, _ = new(big.Int).SetString("da2c742f6aa8accc574ccbfda5ea4440e93dc29cb25c79521ed1544f651494a6", 16)
	const sqrtExponente_nocarry_0257 = "368b1d0bdaaa2b3315d332ff697a91103a4f70a72c971e5487b45513d9452529"
...
func (z *e_nocarry_0257) Sqrt(x *e_nocarry_0257) *e_nocarry_0257 {
	...
	var one, alpha, beta, tx, square e_nocarry_0257
	one.SetOne()
	tx.Double(x)
	alpha.Exp(tx, _bSqrtExponente_nocarry_0257)
	beta.Square(&alpha).
		Mul(&beta, &tx).
		Sub(&beta, &one).
		Mul(&beta, x).
...
const (
	inversionCorrectionFactorWord0 = 10126102480014541907
	inversionCorrectionFactorWord1 = 12990473817386345646
	inversionCorrectionFactorWord2 = 5907691107253043974
	inversionCorrectionFactorWord3 = 5719522383094963375
	inversionCorrectionFactorWord4 = 1
	invIterationsN                 = 18
)
...
func (z *e_nocarry_0257) inverseExp(x *e_nocarry_0257) *e_nocarry_0257 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0257, nBits int) uint64 {
...
// 0 ≤ x, y < 2²⁵⁷
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0257) linearComb(x *e_nocarry_0257, xC int64, y *e_nocarry_0257, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2²⁵⁷ = 2³²¹
	// therefore | hi | < 2¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0257, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0257) mulWNonModular(x *e_nocarry_0257, y int64) uint64 {

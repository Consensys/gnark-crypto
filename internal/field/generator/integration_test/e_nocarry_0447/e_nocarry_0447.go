...
// Modulus q =
//
//	q[base10] = 275687218141557006742445448080300113808533207795808360896025284340949650337615277255956798603851887009356283072734580117284251659655183
//	q[base16] = 0x61198f79ca6565780d0fed35932a6fe2179aa3ebc800ad32c91b216563a41625a26ef3acfb3962e57ab1fff35f234188412ec4f81b3bd80f
//
// # Warning
//
...
const (
	Limbs = 7         // number of 64 bits words needed to represent a e_nocarry_0447
	Bits  = 447       // number of bits needed to represent a e_nocarry_0447
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0447
)

// Field modulus q
const (
	q0 uint64 = 4696908031281846287
	q1 uint64 = 8841128964243276168
	q2 uint64 = 11704560405857264357
	q3 uint64 = 14491212945341814309
	q4 uint64 = 1700852042353913138
	q5 uint64 = 941231661501804514
	q6 uint64 = 6996781249346233720
)
...
const qInvNeg uint64 = 2371392110332995857
...
func init() {
	_modulus.SetString("61198f79ca6565780d0fed35932a6fe2179aa3ebc800ad32c91b216563a41625a26ef3acfb3962e57ab1fff35f234188412ec4f81b3bd80f", 16)
}

// Newe_nocarry_0447 returns a new e_nocarry_0447 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0447
//	v.SetUint64(...)
func Newe_nocarry_0447(v uint64) e_nocarry_0447 {
	z := e_nocarry_0447{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0447) SetInt64(v int64) *e_nocarry_0447 {
...
func (z *e_nocarry_0447) SetInterface(i1 interface{}) (*e_nocarry_0447, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0447 with <nil>")
	}
...
func (z *e_nocarry_0447) SetOne() *e_nocarry_0447 {
	z[0] = 9052928011145859042
	z[1] = 764486145222999279
	z[2] = 13484367335704574517
	z[3] = 7911062256735474612
	z[4] = 15045039989001725338
	z[5] = 16564280750705942587
	z[6] = 4453181575017084175
	return z
}
...
func (z *e_nocarry_0447) NotEqual(x *e_nocarry_0447) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0447) IsOne() bool {
	return (z[6] ^ 4453181575017084175 | z[5] ^ 16564280750705942587 | z[4] ^ 15045039989001725338 | z[3] ^ 7911062256735474612 | z[2] ^ 13484367335704574517 | z[1] ^ 764486145222999279 | z[0] ^ 9052928011145859042) == 0
}
...
func (z *e_nocarry_0447) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0447) FitsOnOneWord() bool {
...
func (z *e_nocarry_0447) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 2348454015640923144, 0)
	_, b = bits.Sub64(_z[1], 13643936518976413892, b)
	_, b = bits.Sub64(_z[2], 15075652239783407986, b)
	_, b = bits.Sub64(_z[3], 7245606472670907154, b)
	_, b = bits.Sub64(_z[4], 850426021176956569, b)
	_, b = bits.Sub64(_z[5], 470615830750902257, b)
	_, b = bits.Sub64(_z[6], 3498390624673116860, b)
...
func (z *e_nocarry_0447) smallerThanModulus() bool {
...
func (z *e_nocarry_0447) Select(c int, x0 *e_nocarry_0447, x1 *e_nocarry_0447) *e_nocarry_0447 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0447) Exp(x e_nocarry_0447, k *big.Int) *e_nocarry_0447 {
...
func (z *e_nocarry_0447) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0447) SetString(number string) (*e_nocarry_0447, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0447.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0447) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0447.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0447) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0447.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0447, _ = new(big.Int).SetString("308cc7bce532b2bc0687f69ac99537f10bcd51f5e4005699648d90b2b1d20b12d13779d67d9cb172bd58fff9af91a0c42097627c0d9dec07", 16)
	const sqrtExponente_nocarry_0447 = "184663de7299595e0343fb4d64ca9bf885e6a8faf2002b4cb246c85958e90589689bbceb3ece58b95eac7ffcd7c8d062104bb13e06cef604"
...
func (z *e_nocarry_0447) Sqrt(x *e_nocarry_0447) *e_nocarry_0447 {
	...
	var y, square e_nocarry_0447
	y.Exp(*x, _bSqrtExponente_nocarry_0447)
...
const (
	inversionCorrectionFactorWord0 = 16191129544280049689
	inversionCorrectionFactorWord1 = 17111776642357200645
	inversionCorrectionFactorWord2 = 5972881093619207168
	inversionCorrectionFactorWord3 = 13506210516327196151
	inversionCorrectionFactorWord4 = 6858299027940554345
	inversionCorrectionFactorWord5 = 15993972938810418763
	inversionCorrectionFactorWord6 = 6250544785032380912
	invIterationsN                 = 30
)
...
func (z *e_nocarry_0447) inverseExp(x *e_nocarry_0447) *e_nocarry_0447 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0447, nBits int) uint64 {
...
// 0 ≤ x, y < 2⁴⁴⁷
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0447) linearComb(x *e_nocarry_0447, xC int64, y *e_nocarry_0447, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2⁴⁴⁷ = 2⁵¹¹
	// therefore | hi | < 2⁶³ ≤ 2⁶³
...
func negL(x *e_nocarry_0447, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0447) mulWNonModular(x *e_nocarry_0447, y int64) uint64 {

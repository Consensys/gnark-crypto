...
// Modulus q =
//
//	q[base10] = 208231523860067378499192026133006407836540853240604736713854590476195704349144435553158814221793
//	q[base16] = 0x18f4ecd2253cffcea3fe43c696eaac05da432ab4e0a1486f6bfc96cfdf67d37b358f0773704bc5e1
//
// # Warning
//
...
const (
	Limbs = 5         // number of 64 bits words needed to represent a e_nocarry_0317
	Bits  = 317       // number of bits needed to represent a e_nocarry_0317
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0317
)

// Field modulus q
const (
	q0 uint64 = 3859311598066451937
	q1 uint64 = 7781260075739894651
	q2 uint64 = 15727461280059050095
	q3 uint64 = 11816956992481307653
	q4 uint64 = 1798322538539712462
)
...
const qInvNeg uint64 = 16341399561699377631
...
func init() {
	_modulus.SetString("18f4ecd2253cffcea3fe43c696eaac05da432ab4e0a1486f6bfc96cfdf67d37b358f0773704bc5e1", 16)
}

// Newe_nocarry_0317 returns a new e_nocarry_0317 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0317
//	v.SetUint64(...)
func Newe_nocarry_0317(v uint64) e_nocarry_0317 {
	z := e_nocarry_0317{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0317) SetInt64(v int64) *e_nocarry_0317 {
...
func (z *e_nocarry_0317) SetInterface(i1 interface{}) (*e_nocarry_0317, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0317 with <nil>")
	}
...
func (z *e_nocarry_0317) SetOne() *e_nocarry_0317 {
	z[0] = 16747116240464135478
	z[1] = 14421119611148811567
	z[2] = 8746083862795463589
	z[3] = 10957638591153784773
	z[4] = 463518688312426989
	return z
}
...
func (z *e_nocarry_0317) NotEqual(x *e_nocarry_0317) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0317) IsOne() bool {
	return (z[4] ^ 463518688312426989 | z[3] ^ 10957638591153784773 | z[2] ^ 8746083862795463589 | z[1] ^ 14421119611148811567 | z[0] ^ 16747116240464135478) == 0
}
...
func (z *e_nocarry_0317) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0317) FitsOnOneWord() bool {
...
func (z *e_nocarry_0317) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 11153027835888001777, 0)
	_, b = bits.Sub64(_z[1], 13114002074724723133, b)
	_, b = bits.Sub64(_z[2], 17087102676884300855, b)
	_, b = bits.Sub64(_z[3], 5908478496240653826, b)
	_, b = bits.Sub64(_z[4], 899161269269856231, b)
...
func (z *e_nocarry_0317) smallerThanModulus() bool {
...
func (z *e_nocarry_0317) Select(c int, x0 *e_nocarry_0317, x1 *e_nocarry_0317) *e_nocarry_0317 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0317) Exp(x e_nocarry_0317, k *big.Int) *e_nocarry_0317 {
...
func (z *e_nocarry_0317) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0317) SetString(number string) (*e_nocarry_0317, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0317.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0317) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0317.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0317) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0317.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0317, _ = new(big.Int).SetString("c7a7669129e7fe751ff21e34b755602ed21955a7050a437b5fe4b67efb3e9bd9ac783b9b825e2f0", 16)
	const sqrtExponente_nocarry_0317 = "63d3b34894f3ff3a8ff90f1a5baab017690caad3828521bdaff25b3f7d9f4decd63c1dcdc12f17"
...
func (z *e_nocarry_0317) Sqrt(x *e_nocarry_0317) *e_nocarry_0317 {
	...
	var y, b, t, w e_nocarry_0317
	// w = x^((s-1)/2))
	w.Exp(*x, _bSqrtExponente_nocarry_0317)
	...
	var g = e_nocarry_0317{
		10366824945162804519,
		8741922659670872747,
		17119554971363146097,
		9819545639630226578,
		1081361976634033555,
	}
	r := uint64(5)
...
const (
	inversionCorrectionFactorWord0 = 5189768737261148213
	inversionCorrectionFactorWord1 = 13891646229037174058
	inversionCorrectionFactorWord2 = 5178881626997867363
	inversionCorrectionFactorWord3 = 12987713876433603830
	inversionCorrectionFactorWord4 = 1757846583274673116
	invIterationsN                 = 22
)
...
func (z *e_nocarry_0317) inverseExp(x *e_nocarry_0317) *e_nocarry_0317 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0317, nBits int) uint64 {
...
// 0 ≤ x, y < 2³¹⁷
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0317) linearComb(x *e_nocarry_0317, xC int64, y *e_nocarry_0317, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³¹⁷ = 2³⁸¹
	// therefore | hi | < 2⁶¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0317, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0317) mulWNonModular(x *e_nocarry_0317, y int64) uint64 {

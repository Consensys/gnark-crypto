...
const (
	Limbs = 7         // number of 64 bits words needed to represent a e_nocarry_0445
	Bits  = 445       // number of bits needed to represent a e_nocarry_0445
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0445
)

// Field modulus q
const (
	q0 uint64 = 5880097284428511215
	q1 uint64 = 2932639606349802227
	q2 uint64 = 13997982729141892023
	q3 uint64 = 16562434220547035011
	q4 uint64 = 5135534796861997794
	q5 uint64 = 16826209202473826658
	q6 uint64 = 2153607204310815611
)
...
const qInvNeg uint64 = 501559695169530097
...
func init() {
	_modulus.SetString("1de326578eab137be982b41edddc7962474515b0429e02e2e5d9962c0b7acf83c242d320ae8e33b728b2d42f26b8aaf3519a4d537b19b7ef", 16)
}

// Newe_nocarry_0445 returns a new e_nocarry_0445 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0445
//	v.SetUint64(...)
func Newe_nocarry_0445(v uint64) e_nocarry_0445 {
	z := e_nocarry_0445{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0445) SetInt64(v int64) *e_nocarry_0445 {
...
func (z *e_nocarry_0445) SetInterface(i1 interface{}) (*e_nocarry_0445, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0445 with <nil>")
	}
...
func (z *e_nocarry_0445) SetOne() *e_nocarry_0445 {
	z[0] = 8299453945700565128
	z[1] = 13432371296620685413
	z[2] = 17143346682831725126
	z[3] = 15074478825300132833
	z[4] = 14255953846232672488
	z[5] = 12964278969885799661
	z[6] = 1217886439223026720
	return z
}
...
func (z *e_nocarry_0445) NotEqual(x *e_nocarry_0445) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0445) IsOne() bool {
	return (z[6] ^ 1217886439223026720 | z[5] ^ 12964278969885799661 | z[4] ^ 14255953846232672488 | z[3] ^ 15074478825300132833 | z[2] ^ 17143346682831725126 | z[1] ^ 13432371296620685413 | z[0] ^ 8299453945700565128) == 0
}
...
func (z *e_nocarry_0445) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0445) FitsOnOneWord() bool {
...
func (z *e_nocarry_0445) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 12163420679069031416, 0)
	_, b = bits.Sub64(_z[1], 10689691840029676921, b)
	_, b = bits.Sub64(_z[2], 16222363401425721819, b)
	_, b = bits.Sub64(_z[3], 8281217110273517505, b)
	_, b = bits.Sub64(_z[4], 2567767398430998897, b)
	_, b = bits.Sub64(_z[5], 17636476638091689137, b)
	_, b = bits.Sub64(_z[6], 1076803602155407805, b)
...
func (z *e_nocarry_0445) smallerThanModulus() bool {
...
func (z *e_nocarry_0445) Select(c int, x0 *e_nocarry_0445, x1 *e_nocarry_0445) *e_nocarry_0445 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0445) Exp(x e_nocarry_0445, k *big.Int) *e_nocarry_0445 {
...
var rSquare = e_nocarry_0445{
	3484858108718755102,
	17691700352749771205,
	10504027440485370610,
	8899394935147438983,
	7518098645832200656,
	15948024819366495347,
	1201995622468056403,
}
...
func (z *e_nocarry_0445) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0445) SetString(number string) (*e_nocarry_0445, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0445.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0445) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0445.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0445) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0445.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0445, _ = new(big.Int).SetString("ef1932bc75589bdf4c15a0f6eee3cb123a28ad8214f017172eccb1605bd67c1e1216990574719db94596a17935c5579a8cd26a9bd8cdbf7", 16)
	const sqrtExponente_nocarry_0445 = "778c995e3aac4defa60ad07b7771e5891d1456c10a780b8b976658b02deb3e0f090b4c82ba38cedca2cb50bc9ae2abcd4669354dec66dfc"
...
const (
	inversionCorrectionFactorWord0 = 4209689251648545721
	inversionCorrectionFactorWord1 = 13323301489638076551
	inversionCorrectionFactorWord2 = 18442133904246318143
	inversionCorrectionFactorWord3 = 14037698763263912559
	inversionCorrectionFactorWord4 = 12240492238160699793
	inversionCorrectionFactorWord5 = 2005483162792805037
	inversionCorrectionFactorWord6 = 89856002991966771
	invIterationsN                 = 30
)
...
func (z *e_nocarry_0445) inverseExp(x *e_nocarry_0445) *e_nocarry_0445 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0445, nBits int) uint64 {
...
// 0 ≤ x, y < 2⁴⁴⁵
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0445) linearComb(x *e_nocarry_0445, xC int64, y *e_nocarry_0445, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2⁴⁴⁵ = 2⁵⁰⁹
	// therefore | hi | < 2⁶¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0445, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0445) mulWNonModular(x *e_nocarry_0445, y int64) uint64 {

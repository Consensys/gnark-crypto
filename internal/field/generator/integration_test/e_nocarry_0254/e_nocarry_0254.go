...
// Modulus q =
//
//	q[base10] = 22428168551571006072240331127965397453601185182898944983193630028201504861903
//	q[base16] = 0x3195e4bf66c30c73019028785c299070b66514d310239bde54e0f145b7f0eecf
//
// # Warning
//
...
const (
	Limbs = 4         // number of 64 bits words needed to represent a e_nocarry_0254
	Bits  = 254       // number of bits needed to represent a e_nocarry_0254
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0254
)

// Field modulus q
const (
	q0 uint64 = 6116153575710191311
	q1 uint64 = 13142933984292084702
	q2 uint64 = 112634488091676784
	q3 uint64 = 3573013390102301811
)
...
const qInvNeg uint64 = 1043686821029783505
...
func init() {
	_modulus.SetString("3195e4bf66c30c73019028785c299070b66514d310239bde54e0f145b7f0eecf", 16)
}

// Newe_nocarry_0254 returns a new e_nocarry_0254 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0254
//	v.SetUint64(...)
func Newe_nocarry_0254(v uint64) e_nocarry_0254 {
	z := e_nocarry_0254{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0254) SetInt64(v int64) *e_nocarry_0254 {
...
func (z *e_nocarry_0254) SetInterface(i1 interface{}) (*e_nocarry_0254, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0254 with <nil>")
	}
...
func (z *e_nocarry_0254) SetOne() *e_nocarry_0254 {
	z[0] = 6312720268868146677
	z[1] = 8072306373377782952
	z[2] = 17883571633251167692
	z[3] = 581677123198042560
	return z
}
...
func (z *e_nocarry_0254) NotEqual(x *e_nocarry_0254) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0254) IsOne() bool {
	return (z[3] ^ 581677123198042560 | z[2] ^ 17883571633251167692 | z[1] ^ 8072306373377782952 | z[0] ^ 6312720268868146677) == 0
}
...
func (z *e_nocarry_0254) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0254) FitsOnOneWord() bool {
...
func (z *e_nocarry_0254) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 3058076787855095656, 0)
	_, b = bits.Sub64(_z[1], 6571466992146042351, b)
	_, b = bits.Sub64(_z[2], 9279689280900614200, b)
	_, b = bits.Sub64(_z[3], 1786506695051150905, b)
...
func (z *e_nocarry_0254) smallerThanModulus() bool {
...
func (z *e_nocarry_0254) Select(c int, x0 *e_nocarry_0254, x1 *e_nocarry_0254) *e_nocarry_0254 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0254) Exp(x e_nocarry_0254, k *big.Int) *e_nocarry_0254 {
...
func (z *e_nocarry_0254) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0254) SetString(number string) (*e_nocarry_0254, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0254.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0254) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0254.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0254) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0254.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0254, _ = new(big.Int).SetString("18caf25fb361863980c8143c2e14c8385b328a698811cdef2a7078a2dbf87767", 16)
	const sqrtExponente_nocarry_0254 = "c65792fd9b0c31cc0640a1e170a641c2d994534c408e6f795383c516dfc3bb4"
...
const (
	inversionCorrectionFactorWord0 = 14520308175709734680
	inversionCorrectionFactorWord1 = 12810580567403220710
	inversionCorrectionFactorWord2 = 17786673612985477402
	inversionCorrectionFactorWord3 = 415750791404904752
	invIterationsN                 = 18
)
...
func (z *e_nocarry_0254) inverseExp(x *e_nocarry_0254) *e_nocarry_0254 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0254, nBits int) uint64 {
...
// 0 ≤ x, y < 2²⁵⁴
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0254) linearComb(x *e_nocarry_0254, xC int64, y *e_nocarry_0254, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2²⁵⁴ = 2³¹⁸
	// therefore | hi | < 2⁶² ≤ 2⁶³
...
func negL(x *e_nocarry_0254, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0254) mulWNonModular(x *e_nocarry_0254, y int64) uint64 {

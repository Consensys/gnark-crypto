...
// Modulus q =
//
//	q[base10] = 103522049291640975677807246044039261285672503094918216779705074199160577420681
//	q[base16] = 0xe4df674ba4a97d6dfbcf3aa42652a5fb78c3a859d229b2c8042f33ac041d6d89
//
// # Warning
//
...
const (
	Limbs = 4         // number of 64 bits words needed to represent a e_cios_0256
	Bits  = 256       // number of bits needed to represent a e_cios_0256
	Bytes = Limbs * 8 // number of bytes needed to represent a e_cios_0256
)

// Field modulus q
const (
	q0 uint64 = 301516513953541513
	q1 uint64 = 8701984008741434056
	q2 uint64 = 18144785900391015931
	q3 uint64 = 16492013935036824941
)
...
const qInvNeg uint64 = 15418389418293126983
...
func init() {
	_modulus.SetString("e4df674ba4a97d6dfbcf3aa42652a5fb78c3a859d229b2c8042f33ac041d6d89", 16)
}

// Newe_cios_0256 returns a new e_cios_0256 from a uint64 value
//
// it is equivalent to
//
//	var v e_cios_0256
//	v.SetUint64(...)
func Newe_cios_0256(v uint64) e_cios_0256 {
	z := e_cios_0256{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_cios_0256) SetInt64(v int64) *e_cios_0256 {
...
func (z *e_cios_0256) SetInterface(i1 interface{}) (*e_cios_0256, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_cios_0256 with <nil>")
	}
...
func (z *e_cios_0256) SetOne() *e_cios_0256 {
	z[0] = 18145227559756010103
	z[1] = 9744760064968117559
	z[2] = 301958173318535684
	z[3] = 1954730138672726674
	return z
}
...
func (z *e_cios_0256) NotEqual(x *e_cios_0256) uint64 {
...
// IsOne returns z == 1
func (z *e_cios_0256) IsOne() bool {
	return (z[3] ^ 1954730138672726674 | z[2] ^ 301958173318535684 | z[1] ^ 9744760064968117559 | z[0] ^ 18145227559756010103) == 0
}
...
func (z *e_cios_0256) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_cios_0256) FitsOnOneWord() bool {
...
func (z *e_cios_0256) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 150758256976770757, 0)
	_, b = bits.Sub64(_z[1], 13574364041225492836, b)
	_, b = bits.Sub64(_z[2], 18295764987050283773, b)
	_, b = bits.Sub64(_z[3], 8246006967518412470, b)
...
func (z *e_cios_0256) smallerThanModulus() bool {
...
func (z *e_cios_0256) Select(c int, x0 *e_cios_0256, x1 *e_cios_0256) *e_cios_0256 {
...
// Exp z = xᵏ (mod q)
func (z *e_cios_0256) Exp(x e_cios_0256, k *big.Int) *e_cios_0256 {
...
func (z *e_cios_0256) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_cios_0256) SetString(number string) (*e_cios_0256, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_cios_0256.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_cios_0256) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_cios_0256.SetString for valid prefixes (0x, 0b, ...)
func (z *e_cios_0256) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_cios_0256.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_cios_0256, _ = new(big.Int).SetString("726fb3a5d254beb6fde79d52132952fdbc61d42ce914d964021799d6020eb6c4", 16)
	const sqrtExponente_cios_0256 = "e4df674ba4a97d6dfbcf3aa42652a5fb78c3a859d229b2c8042f33ac041d6d8"

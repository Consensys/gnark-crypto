...
const (
	Limbs = 7         // number of 64 bits words needed to represent a e_nocarry_0385
	Bits  = 385       // number of bits needed to represent a e_nocarry_0385
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0385
)

// Field modulus q
const (
	q0 uint64 = 3664296589812790295
	q1 uint64 = 10067275827436206460
	q2 uint64 = 3675593886699735335
	q3 uint64 = 16646076165771905157
	q4 uint64 = 4695203266885035275
	q5 uint64 = 17922389949167841555
	q6 uint64 = 1
)
...
const qInvNeg uint64 = 8149737536866942041
...
func init() {
	_modulus.SetString("1f8b91eb3ae2e31134128b67eb6efa50be702be13fcee48853302552a7355c9278bb62606be36397c32da32558b777417", 16)
}

// Newe_nocarry_0385 returns a new e_nocarry_0385 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0385
//	v.SetUint64(...)
func Newe_nocarry_0385(v uint64) e_nocarry_0385 {
	z := e_nocarry_0385{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0385) SetInt64(v int64) *e_nocarry_0385 {
...
func (z *e_nocarry_0385) SetInterface(i1 interface{}) (*e_nocarry_0385, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0385 with <nil>")
	}
...
func (z *e_nocarry_0385) SetOne() *e_nocarry_0385 {
	z[0] = 360648981283868183
	z[1] = 8461421898057289105
	z[2] = 2574647824835468312
	z[3] = 11225340675865658769
	z[4] = 14344889717619253954
	z[5] = 2072189072142138839
	z[6] = 1
	return z
}
...
func (z *e_nocarry_0385) NotEqual(x *e_nocarry_0385) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0385) IsOne() bool {
	return (z[6] ^ 1 | z[5] ^ 2072189072142138839 | z[4] ^ 14344889717619253954 | z[3] ^ 11225340675865658769 | z[2] ^ 2574647824835468312 | z[1] ^ 8461421898057289105 | z[0] ^ 360648981283868183) == 0
}
...
func (z *e_nocarry_0385) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0385) FitsOnOneWord() bool {
...
func (z *e_nocarry_0385) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 1832148294906395148, 0)
	_, b = bits.Sub64(_z[1], 14257009950572879038, b)
	_, b = bits.Sub64(_z[2], 11061168980204643475, b)
	_, b = bits.Sub64(_z[3], 17546410119740728386, b)
	_, b = bits.Sub64(_z[4], 11570973670297293445, b)
	_, b = bits.Sub64(_z[5], 18184567011438696585, b)
...
func (z *e_nocarry_0385) smallerThanModulus() bool {
...
func (z *e_nocarry_0385) Select(c int, x0 *e_nocarry_0385, x1 *e_nocarry_0385) *e_nocarry_0385 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0385) Exp(x e_nocarry_0385, k *big.Int) *e_nocarry_0385 {
...
var rSquare = e_nocarry_0385{
	7683182733581561985,
	12747468535779558383,
	6660442295994196049,
	4964688928305039701,
	8582938909705126846,
	15039629904858942011,
	0,
}
...
func (z *e_nocarry_0385) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0385) SetString(number string) (*e_nocarry_0385, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0385.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0385) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0385.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0385) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0385.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0385, _ = new(big.Int).SetString("fc5c8f59d7171889a0945b3f5b77d285f3815f09fe77244299812a9539aae493c5db13035f1b1cbe196d192ac5bbba0b", 16)
	const sqrtExponente_nocarry_0385 = "7e2e47aceb8b8c44d04a2d9fadbbe942f9c0af84ff3b92214cc0954a9cd57249e2ed8981af8d8e5f0cb68c9562dddd06"
...
func (z *e_nocarry_0385) Sqrt(x *e_nocarry_0385) *e_nocarry_0385 {
	...
	var y, square e_nocarry_0385
	y.Exp(*x, _bSqrtExponente_nocarry_0385)
...
const (
	inversionCorrectionFactorWord0 = 13096052450683349539
	inversionCorrectionFactorWord1 = 1301040583413393616
	inversionCorrectionFactorWord2 = 9761879260830374204
	inversionCorrectionFactorWord3 = 8852460227825239947
	inversionCorrectionFactorWord4 = 4923620352760959563
	inversionCorrectionFactorWord5 = 15978522223960069084
	inversionCorrectionFactorWord6 = 1
	invIterationsN                 = 26
)
...
func (z *e_nocarry_0385) inverseExp(x *e_nocarry_0385) *e_nocarry_0385 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0385, nBits int) uint64 {
...
// 0 ≤ x, y < 2³⁸⁵
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0385) linearComb(x *e_nocarry_0385, xC int64, y *e_nocarry_0385, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³⁸⁵ = 2⁴⁴⁹
	// therefore | hi | < 2¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0385, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0385) mulWNonModular(x *e_nocarry_0385, y int64) uint64 {

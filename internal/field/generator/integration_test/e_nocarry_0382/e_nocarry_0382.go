...
const (
	Limbs = 6         // number of 64 bits words needed to represent a e_nocarry_0382
	Bits  = 382       // number of bits needed to represent a e_nocarry_0382
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0382
)

// Field modulus q
const (
	q0 uint64 = 4391908625142436673
	q1 uint64 = 10750505160387169489
	q2 uint64 = 13639955452913363021
	q3 uint64 = 10710268983419489108
	q4 uint64 = 972199714876573369
	q5 uint64 = 3503964112984167636
)
...
const qInvNeg uint64 = 9759852500937622335
...
func init() {
	_modulus.SetString("30a094cbc42354d40d7df27d5ff446b994a284e51d9a4b54bd4adb37830da84d9531777c18e18cd13cf331a00f96d341", 16)
}

// Newe_nocarry_0382 returns a new e_nocarry_0382 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0382
//	v.SetUint64(...)
func Newe_nocarry_0382(v uint64) e_nocarry_0382 {
	z := e_nocarry_0382{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0382) SetInt64(v int64) *e_nocarry_0382 {
...
func (z *e_nocarry_0382) SetInterface(i1 interface{}) (*e_nocarry_0382, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0382 with <nil>")
	}
...
func (z *e_nocarry_0382) SetOne() *e_nocarry_0382 {
	z[0] = 14933945021706919867
	z[1] = 1587706419192807401
	z[2] = 5587199030271391356
	z[3] = 1788887304031209304
	z[4] = 13585745499326684768
	z[5] = 926923508788713435
	return z
}
...
func (z *e_nocarry_0382) NotEqual(x *e_nocarry_0382) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0382) IsOne() bool {
	return (z[5] ^ 926923508788713435 | z[4] ^ 13585745499326684768 | z[3] ^ 1788887304031209304 | z[2] ^ 5587199030271391356 | z[1] ^ 1587706419192807401 | z[0] ^ 14933945021706919867) == 0
}
...
func (z *e_nocarry_0382) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0382) FitsOnOneWord() bool {
...
func (z *e_nocarry_0382) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 11419326349425994145, 0)
	_, b = bits.Sub64(_z[1], 14598624617048360552, b)
	_, b = bits.Sub64(_z[2], 6819977726456681510, b)
	_, b = bits.Sub64(_z[3], 14578506528564520362, b)
	_, b = bits.Sub64(_z[4], 486099857438286684, b)
	_, b = bits.Sub64(_z[5], 1751982056492083818, b)
...
func (z *e_nocarry_0382) smallerThanModulus() bool {
...
func (z *e_nocarry_0382) Select(c int, x0 *e_nocarry_0382, x1 *e_nocarry_0382) *e_nocarry_0382 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0382) Exp(x e_nocarry_0382, k *big.Int) *e_nocarry_0382 {
...
func (z *e_nocarry_0382) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0382) SetString(number string) (*e_nocarry_0382, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0382.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0382) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0382.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0382) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0382.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0382, _ = new(big.Int).SetString("18504a65e211aa6a06bef93eaffa235cca5142728ecd25aa5ea56d9bc186d426ca98bbbe0c70c6689e7998d007cb69a0", 16)
	const sqrtExponente_nocarry_0382 = "614129978846a9a81afbe4fabfe88d73294509ca3b3496a97a95b66f061b509b2a62eef831c319a279e663401f2da6"
...
func (z *e_nocarry_0382) Sqrt(x *e_nocarry_0382) *e_nocarry_0382 {
	...
	var y, b, t, w e_nocarry_0382
	// w = x^((s-1)/2))
	w.Exp(*x, _bSqrtExponente_nocarry_0382)
...
const (
	inversionCorrectionFactorWord0 = 16577352982167520157
	inversionCorrectionFactorWord1 = 10133403681846518726
	inversionCorrectionFactorWord2 = 8344942154723360870
	inversionCorrectionFactorWord3 = 4929189370401828313
	inversionCorrectionFactorWord4 = 3559502540371728848
	inversionCorrectionFactorWord5 = 506772474786932995
	invIterationsN                 = 26
)
...
func (z *e_nocarry_0382) inverseExp(x *e_nocarry_0382) *e_nocarry_0382 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0382, nBits int) uint64 {
...
// 0 ≤ x, y < 2³⁸²
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0382) linearComb(x *e_nocarry_0382, xC int64, y *e_nocarry_0382, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³⁸² = 2⁴⁴⁶
	// therefore | hi | < 2⁶² ≤ 2⁶³
...
func negL(x *e_nocarry_0382, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0382) mulWNonModular(x *e_nocarry_0382, y int64) uint64 {

...
// Modulus q =
//
//	q[base10] = 3594627562344924470403735711098449090421815364980170770040631278621915236763792830976816911217553
//	q[base16] = 0x1aed1c44849f526dcae0d681c738bdf0f50b5de1789910abe6001b98edb119ada776040da6cd3f391
//
// # Warning
//
...
const (
	Limbs = 6         // number of 64 bits words needed to represent a e_nocarry_0321
	Bits  = 321       // number of bits needed to represent a e_nocarry_0321
	Bytes = Limbs * 8 // number of bytes needed to represent a e_nocarry_0321
)

// Field modulus q
const (
	q0 uint64 = 8601946595150525329
	q1 uint64 = 6918014525829651162
	q2 uint64 = 5815798686492461758
	q3 uint64 = 12541795008703618831
	q4 uint64 = 12597065447489480412
	q5 uint64 = 1
)
...
const qInvNeg uint64 = 10066926200886841999
...
func init() {
	_modulus.SetString("1aed1c44849f526dcae0d681c738bdf0f50b5de1789910abe6001b98edb119ada776040da6cd3f391", 16)
}

// Newe_nocarry_0321 returns a new e_nocarry_0321 from a uint64 value
//
// it is equivalent to
//
//	var v e_nocarry_0321
//	v.SetUint64(...)
func Newe_nocarry_0321(v uint64) e_nocarry_0321 {
	z := e_nocarry_0321{v}
	z.Mul(&z, &rSquare)
	return z
}
...
func (z *e_nocarry_0321) SetInt64(v int64) *e_nocarry_0321 {
...
func (z *e_nocarry_0321) SetInterface(i1 interface{}) (*e_nocarry_0321, error) {
	if i1 == nil {
		return nil, errors.New("can't set integration.e_nocarry_0321 with <nil>")
	}
...
func (z *e_nocarry_0321) SetOne() *e_nocarry_0321 {
	z[0] = 5525359343336254331
	z[1] = 1727257462783163334
	z[2] = 11997630018987394048
	z[3] = 8117151867849346051
	z[4] = 14396255697615626774
	z[5] = 0
	return z
}
...
func (z *e_nocarry_0321) NotEqual(x *e_nocarry_0321) uint64 {
...
// IsOne returns z == 1
func (z *e_nocarry_0321) IsOne() bool {
	return (z[5] ^ 0 | z[4] ^ 14396255697615626774 | z[3] ^ 8117151867849346051 | z[2] ^ 11997630018987394048 | z[1] ^ 1727257462783163334 | z[0] ^ 5525359343336254331) == 0
}
...
func (z *e_nocarry_0321) Uint64() uint64 {
	zz := *z
	zz.FromMont()
	return zz[0]
}
...
//
// It is the responsibility of the caller to convert from Montgomery to Regular form if needed.
func (z *e_nocarry_0321) FitsOnOneWord() bool {
...
func (z *e_nocarry_0321) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 4300973297575262665, 0)
	_, b = bits.Sub64(_z[1], 3459007262914825581, b)
	_, b = bits.Sub64(_z[2], 12131271380101006687, b)
	_, b = bits.Sub64(_z[3], 6270897504351809415, b)
	_, b = bits.Sub64(_z[4], 15521904760599516014, b)
...
func (z *e_nocarry_0321) smallerThanModulus() bool {
...
func (z *e_nocarry_0321) Select(c int, x0 *e_nocarry_0321, x1 *e_nocarry_0321) *e_nocarry_0321 {
...
// Exp z = xᵏ (mod q)
func (z *e_nocarry_0321) Exp(x e_nocarry_0321, k *big.Int) *e_nocarry_0321 {
...
func (z *e_nocarry_0321) Text(base int) string {
	if base < 2 || base > 36 {
		panic("invalid base")
	}
	if z == nil {
...
func (z *e_nocarry_0321) SetString(number string) (*e_nocarry_0321, error) {
	// get temporary big int from the pool
	vv := bigIntPool.Get().(*big.Int)

	if _, ok := vv.SetString(number, 0); !ok {
		return nil, errors.New("e_nocarry_0321.SetString failed -> can't parse number into a big.Int " + number)
	}
...
func (z *e_nocarry_0321) MarshalJSON() ([]byte, error) {
	if z == nil {
...
// See e_nocarry_0321.SetString for valid prefixes (0x, 0b, ...)
func (z *e_nocarry_0321) UnmarshalJSON(data []byte) error {
	s := string(data)
	if len(s) > Bits*3 {
		return errors.New("value too large (max = e_nocarry_0321.Bits * 3)")
	}
...
func init() {
	_bLegendreExponente_nocarry_0321, _ = new(big.Int).SetString("d768e22424fa936e5706b40e39c5ef87a85aef0bc4c8855f3000dcc76d88cd6d3bb0206d3669f9c8", 16)
	const sqrtExponente_nocarry_0321 = "d768e22424fa936e5706b40e39c5ef87a85aef0bc4c8855f3000dcc76d88cd6d3bb0206d3669f9c"
...
func (z *e_nocarry_0321) Sqrt(x *e_nocarry_0321) *e_nocarry_0321 {
	...
	var g = e_nocarry_0321{
		10628352437034903393,
		10489534791136280816,
		11093782790395834081,
		14330286519900812156,
		7517079831669561856,
		0,
	}
	r := uint64(4)
...
const (
	inversionCorrectionFactorWord0 = 2161811751340278108
	inversionCorrectionFactorWord1 = 15062139372041452783
	inversionCorrectionFactorWord2 = 5522073829914864174
	inversionCorrectionFactorWord3 = 2694111845222385411
	inversionCorrectionFactorWord4 = 15070046584317015605
...
func (z *e_nocarry_0321) inverseExp(x *e_nocarry_0321) *e_nocarry_0321 {
	qMinusTwo := Modulus()
...
func approximate(x *e_nocarry_0321, nBits int) uint64 {
...
// 0 ≤ x, y < 2³²¹
// |xC|, |yC| < 2⁶³
func (z *e_nocarry_0321) linearComb(x *e_nocarry_0321, xC int64, y *e_nocarry_0321, yC int64) {
	// | (hi, z) | < 2 * 2⁶³ * 2³²¹ = 2³⁸⁵
	// therefore | hi | < 2¹ ≤ 2⁶³
...
func negL(x *e_nocarry_0321, xHi uint64) uint64 {
	var b uint64
...
func (z *e_nocarry_0321) mulWNonModular(x *e_nocarry_0321, y int64) uint64 {

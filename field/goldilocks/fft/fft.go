...
func (domain *Domain) FFT(a []goldilocks.Element, decimation Decimation, opts ...Option) {
	...
	if opt.coset {
		if decimation == DIT {
			...
			if !domain.withPrecompute {
				...
				cosetTable = make([]goldilocks.Element, len(a))
			...
			if domain.withPrecompute {
				parallel.Execute(len(a), func(start, end int) {
					v1 := goldilocks.Vector(a[start:end])
					v2 := goldilocks.Vector(domain.cosetTable[start:end])
					v1.Mul(v1, v2)
				}, opt.nbTasks)
			} else {
				c := domain.FrMultiplicativeGen
				parallel.Execute(len(a), func(start, end int) {
					var at goldilocks.Element
	...
	if !domain.withPrecompute {
		...
		if nbStages-twiddlesStartStage > 0 {
			twiddles = make([][]goldilocks.Element, nbStages-twiddlesStartStage)
			w := domain.Generator
...
func (domain *Domain) FFTInverse(a []goldilocks.Element, decimation Decimation, opts ...Option) {
	...
	if !domain.withPrecompute {
		...
		nbStages := int(bits.TrailingZeros64(domain.Cardinality))
		if nbStages-twiddlesStartStage > 0 {
			twiddlesInv = make([][]goldilocks.Element, nbStages-twiddlesStartStage)
	...
	if !domain.withPrecompute {
		// we need to build the full table or do a bit reverse dance.
		cosetTableInv = make([]goldilocks.Element, len(a))
...
func difFFT(a []goldilocks.Element, w goldilocks.Element, twiddles [][]goldilocks.Element, twiddlesStartStage, stage, maxSplits int, chDone chan struct{}, nbTasks int) {
	if chDone != nil {
		defer close(chDone)
	}
	...
	if stage < twiddlesStartStage {
		if parallelButterfly {
			...
			parallel.Execute(m, func(start, end int) {
				var at goldilocks.Element
				at.Exp(w, big.NewInt(int64(max(start, 1))))
...
func innerDIFWithTwiddlesGeneric(a []goldilocks.Element, twiddles []goldilocks.Element, start, end, m int) {
	goldilocks.Vector(a[start:end]).Butterfly(goldilocks.Vector(a[start+m : end+m]))
	if start == 0 {
		start++
	}
...
func innerDIFWithoutTwiddles(a []goldilocks.Element, at, w goldilocks.Element, start, end, m int) {
	...
	tw := make([]goldilocks.Element, end-start)
	tw[0].Set(&at)
	for i := 1; i < len(tw); i++ {
		tw[i].Mul(&tw[i-1], &w)
	}
	v1 := goldilocks.Vector(a[start+m : end+m])
	v2 := goldilocks.Vector(tw)
	v1.Mul(v1, v2)
}

func ditFFT(a []goldilocks.Element, w goldilocks.Element, twiddles [][]goldilocks.Element, twiddlesStartStage, stage, maxSplits int, chDone chan struct{}, nbTasks int) {
	if chDone != nil {
		defer close(chDone)
	}
	n := len(a)
...
func innerDITWithTwiddlesGeneric(a []goldilocks.Element, twiddles []goldilocks.Element, start, end, m int) {
	twStart := start
	if twStart == 0 {
		twStart = 1
	}
	if twStart < end {
		v1 := goldilocks.Vector(a[twStart+m : end+m])
		v2 := goldilocks.Vector(twiddles[twStart:end])
		v1.Mul(v1, v2)
	}
	goldilocks.Vector(a[start:end]).Butterfly(goldilocks.Vector(a[start+m : end+m]))
}

func innerDITWithoutTwiddles(a []goldilocks.Element, at, w goldilocks.Element, start, end, m int) {
	twStart := start
	if twStart == 0 {
		twStart = 1
	}
	if twStart < end {
		// precompute twiddles for this chunk
		tw := make([]goldilocks.Element, end-twStart)
		tw[0].Set(&at)
		for i := 1; i < len(tw); i++ {
			tw[i].Mul(&tw[i-1], &w)
		}
...
func kerDIFNP_32generic(a []goldilocks.Element, twiddles [][]goldilocks.Element, stage int) {
	...
	for offset := 0; offset < 32; offset += 2 {
		innerDIFWithTwiddlesGeneric(a[offset:offset+2], twiddles[stage+4], 0, 1, 1)
	}
}

func kerDITNP_32generic(a []goldilocks.Element, twiddles [][]goldilocks.Element, stage int) {
	// code unrolled & generated by internal/generator/fft/template/fft.go.tmpl

	for offset := 0; offset < 32; offset += 2 {
		innerDITWithTwiddlesGeneric(a[offset:offset+2], twiddles[stage+4], 0, 1, 1)
	}
	for offset := 0; offset < 32; offset += 4 {
...
func kerDIFNP_256generic(a []goldilocks.Element, twiddles [][]goldilocks.Element, stage int) {
	// code unrolled & generated by internal/generator/fft/template/fft.go.tmpl
	...
	for offset := 0; offset < 256; offset += 2 {
		innerDIFWithTwiddlesGeneric(a[offset:offset+2], twiddles[stage+7], 0, 1, 1)
	}
}

func kerDITNP_256generic(a []goldilocks.Element, twiddles [][]goldilocks.Element, stage int) {
	// code unrolled & generated by internal/generator/fft/template/fft.go.tmpl

	for offset := 0; offset < 256; offset += 2 {
		innerDITWithTwiddlesGeneric(a[offset:offset+2], twiddles[stage+7], 0, 1, 1)
	}
	for offset := 0; offset < 256; offset += 4 {

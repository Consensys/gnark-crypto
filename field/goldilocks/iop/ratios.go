...
func BuildRatioShuffledVectors(numerator, denominator []*Polynomial, beta goldilocks.Element, expectedForm Form, domain *fft.Domain) (*Polynomial, error) {
	...
	coeffs := make([]goldilocks.Element, n)
	t := make([]goldilocks.Element, n)
	coeffs[0].SetOne()
	t[0].SetOne()
	var a, b, c, d goldilocks.Element
...
func BuildRatioCopyConstraint(
	...
	parallel.Execute(n-1, func(start, end int) {
		...
		tInv := goldilocks.BatchInvert(t[start:end])
		for i := start; i < end; i++ {
...
func getSupportIdentityPermutation(nbCopies int, domain *fft.Domain) []goldilocks.Element {
	if nbCopies <= 0 {
		panic("getSupportIdentityPermutation: nbCopies must be positive")
	}

	res := make([]goldilocks.Element, uint64(nbCopies)*domain.Cardinality)

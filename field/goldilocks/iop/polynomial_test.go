...
func randomVector(size int) *[]goldilocks.Element {
	r := make([]goldilocks.Element, size)
	goldilocks.Vector(r).MustSetRandom()
	return &r
}
...
func cmpCoefficents(p, q *goldilocks.Vector) bool {
	if p.Len() != q.Len() {
		return false
	}
	for i := 0; i < p.Len(); i++ {

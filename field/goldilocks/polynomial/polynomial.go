...
type Polynomial []goldilocks.Element
...
func (p *Polynomial) Eval(v *goldilocks.Element) goldilocks.Element {
...
func (p *Polynomial) AddConstantInPlace(c *goldilocks.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Add(&(*p)[i], c)
	}
}
...
func (p *Polynomial) SubConstantInPlace(c *goldilocks.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Sub(&(*p)[i], c)
	}
}
...
func (p *Polynomial) ScaleInPlace(c *goldilocks.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Mul(&(*p)[i], c)
	}
}
...
func (p *Polynomial) Scale(c *goldilocks.Element, p0 Polynomial) {
	if len(*p) != len(p0) {
...
func InterpolateOnRange(v []goldilocks.Element) Polynomial {
	nEvals := uint8(len(v))
	if int(nEvals) != len(v) {
...
func computeLagrangeBasis(domainSize uint8) []Polynomial {
	...
	constTerms := make([]goldilocks.Element, domainSize)
	...
	constTerms = goldilocks.BatchInvert(constTerms)
	for l := uint8(0); l < domainSize; l++ {

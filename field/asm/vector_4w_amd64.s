...
done_14:
    RET
    // mulVec(res, a,b *Element, n uint64) res = a[0...n] * b[0...n]
TEXT ·mulVec(SB), NOSPLIT, $0-32
    ...
    VPCMPEQB Y8, Y8, Y8
    VPMOVZXDQ Y8, Z8
    MOVQ $0x0000000000005555, DX
    KMOVD DX, K1
loop_17:
    TESTQ AX, AX
    JEQ done_16                                           // n == 0, we are done
    VMOVDQU64 256+0*64(R15, AX), Z16
    VMOVDQU64 256+1*64(R15, AX), Z17
    VMOVDQU64 256+2*64(R15, AX), Z18
    VMOVDQU64 256+3*64(R15, AX), Z19
    VMOVDQU64 256+0*64(CX, AX), Z24
    VMOVDQU64 256+1*64(CX, AX), Z25
    VMOVDQU64 256+2*64(CX, AX), Z26
    VMOVDQU64 256+3*64(CX, AX), Z27
    MOVQ 0*8(R15, AX), DX
    MOVQ 0*8(CX, AX), BX
    MOVQ 1*8(CX, AX), SI
    MOVQ 2*8(CX, AX), DI
    ...
    VPERMQ $0xd8, Z22, Z22
    VPERMQ $0xd8, Z23, Z23
    MULXQ $const_qInvNeg, DX, s0-8(SP)
    MULXQ ·qElement+0(SB), R14, s0-8(SP)
    ADDQ R14, R10
    ADCQ s0-8(SP), R11
    MULXQ ·qElement+16(SB), R14, s0-8(SP)
    ADCQ R14, R12
    ADCQ s0-8(SP), R13
    ADCQ $0, R9
    MULXQ ·qElement+8(SB), R14, s0-8(SP)
    ADDQ R14, R11
    ADCQ s0-8(SP), R12
    MULXQ ·qElement+24(SB), R14, s0-8(SP)
    ADCQ R14, R13
    ADCQ s0-8(SP), R9
    ADCQ $0, R10
    MOVQ 1*8(R15, AX), DX
    ...
    VPERMQ $0xd8, Z30, Z30
    VPERMQ $0xd8, Z31, Z31
    MULXQ BX, R14, s0-8(SP)
    ADDQ R14, R11
    ADCQ s0-8(SP), R12
    MULXQ DI, R14, s0-8(SP)
    ADCQ R14, R13
    ADCQ s0-8(SP), R9
    ADCQ $0, R10
    MULXQ SI, R14, s0-8(SP)
    ADDQ R14, R12
    ADCQ s0-8(SP), R13
    MULXQ R8, R14, s0-8(SP)
    ADCQ R14, R9
    ADCQ s0-8(SP), R10
    ADCQ $0, R11
    MOVQ R11, DX

...
const (
	...
	VectorASMFileName  = "vector_%dw_amd64.s"
)
...
func (f *FFAmd64) Define(name string, nbInputs int, fn func(args ...amd64.Register)) func(args ...amd64.Register) {

	inputs := make([]string, nbInputs)
	for i := 0; i < nbInputs; i++ {
		inputs[i] = fmt.Sprintf("in%d", i)
	}
	name = strings.ToUpper(name) + fmt.Sprintf("_%d", f.defineCpt)
	f.defineCpt++
	f.StartDefine()
	...
	return func(args ...amd64.Register) {
...
func GenerateFieldWrapper(w io.Writer, F *config.FieldConfig, asmDirBuildPath, asmDirIncludePath string) error {
	...
	if F.ASMVector {
		toInclude = fmt.Sprintf(VectorASMFileName, F.NbWords)
		if err := hashAndInclude(toInclude); err != nil {
			return err
		}
	}
...
func GenerateVectorASM(w io.Writer, nbWords int) error {
	f := NewFFAmd64(w, nbWords)
	f.WriteLn("")
	f.Comment("Code generated by gnark-crypto/generator. DO NOT EDIT.")
	f.Comment("Partially derived from Dag Arne Osvik's work in github.com/a16z/vectorized-fields")
	f.WriteLn("")

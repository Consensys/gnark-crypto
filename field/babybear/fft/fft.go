...
func (domain *Domain) FFT(a []babybear.Element, decimation Decimation, opts ...Option) {
	...
	if opt.coset {
		if decimation == DIT {
			...
			if domain.withPrecompute {
				parallel.Execute(len(a), func(start, end int) {
					v1 := babybear.Vector(a[start:end])
					v2 := babybear.Vector(domain.cosetTable[start:end])
					v1.Mul(v1, v2)
				}, opt.nbTasks)
			} else {
...
func (domain *Domain) FFTInverse(a []babybear.Element, decimation Decimation, opts ...Option) {
	opt := fftOptions(opts)
	...
	if decimation == DIT {
		if domain.withPrecompute {
			if opt.nbTasks == 1 {
				va := babybear.Vector(a)
				va.Mul(va, babybear.Vector(domain.cosetTableInv))
...
func difFFT(a []babybear.Element, w babybear.Element, twiddles [][]babybear.Element, twiddlesStartStage, stage, maxSplits int, chDone chan struct{}, nbTasks int) {
	if chDone != nil {
		defer close(chDone)
	}
...
func innerDIFWithTwiddlesGeneric(a []babybear.Element, twiddles []babybear.Element, start, end, m int) {
	babybear.Vector(a[start:end]).Butterfly(babybear.Vector(a[start+m : end+m]))
	if start == 0 {
		start++
	}
	v1 := babybear.Vector(a[start+m : end+m])
	v2 := babybear.Vector(twiddles[start:end])
	v1.Mul(v1, v2)
}

func innerDIFWithoutTwiddles(a []babybear.Element, at, w babybear.Element, start, end, m int) {
	babybear.Vector(a[start:end]).Butterfly(babybear.Vector(a[start+m : end+m]))
	if start == 0 {
		start++
	}
...
func innerDITWithTwiddlesGeneric(a []babybear.Element, twiddles []babybear.Element, start, end, m int) {
	twStart := start
	if twStart == 0 {
		twStart = 1
	}
	if twStart < end {
		v1 := babybear.Vector(a[twStart+m : end+m])
		v2 := babybear.Vector(twiddles[twStart:end])
		v1.Mul(v1, v2)
	}
	babybear.Vector(a[start:end]).Butterfly(babybear.Vector(a[start+m : end+m]))
}

func innerDITWithoutTwiddles(a []babybear.Element, at, w babybear.Element, start, end, m int) {
	twStart := start
	if twStart == 0 {
		twStart = 1
	}
	if twStart < end {
		// precompute twiddles for this chunk
		tw := make([]babybear.Element, end-twStart)
		tw[0].Set(&at)
		for i := 1; i < len(tw); i++ {
			tw[i].Mul(&tw[i-1], &w)
		}
...
func kerDIFNP_256generic(a []babybear.Element, twiddles [][]babybear.Element, stage int) {
...
func kerDITNP_256generic(a []babybear.Element, twiddles [][]babybear.Element, stage int) {
	// code unrolled & generated by internal/generator/fft/template/fft.go.tmpl

...
type Polynomial []babybear.Element
...
// returns a babybear.Element
func (p *Polynomial) Eval(v *babybear.Element) babybear.Element {
...
func (p *Polynomial) AddConstantInPlace(c *babybear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Add(&(*p)[i], c)
	}
}
...
func (p *Polynomial) SubConstantInPlace(c *babybear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Sub(&(*p)[i], c)
	}
}
...
func (p *Polynomial) ScaleInPlace(c *babybear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Mul(&(*p)[i], c)
	}
}
...
func (p *Polynomial) Scale(c *babybear.Element, p0 Polynomial) {
	if len(*p) != len(p0) {
...
func InterpolateOnRange(v []babybear.Element) Polynomial {
	nEvals := uint8(len(v))
	if int(nEvals) != len(v) {
...
func computeLagrangeBasis(domainSize uint8) []Polynomial {
	...
	constTerms := make([]babybear.Element, domainSize)
	...
	constTerms = babybear.BatchInvert(constTerms)
	for l := uint8(0); l < domainSize; l++ {

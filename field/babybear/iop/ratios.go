...
func BuildRatioShuffledVectors(numerator, denominator []*Polynomial, beta babybear.Element, expectedForm Form, domain *fft.Domain) (*Polynomial, error) {
	...
	coeffs := make([]babybear.Element, n)
	t := make([]babybear.Element, n)
	coeffs[0].SetOne()
	t[0].SetOne()
	var a, b, c, d babybear.Element
...
func BuildRatioCopyConstraint(
	...
	parallel.Execute(n-1, func(start, end int) {
		...
		tInv := babybear.BatchInvert(t[start:end])
		for i := start; i < end; i++ {
...
func getSupportIdentityPermutation(nbCopies int, domain *fft.Domain) []babybear.Element {
	if nbCopies <= 0 {
		panic("getSupportIdentityPermutation: nbCopies must be positive")
	}

	res := make([]babybear.Element, uint64(nbCopies)*domain.Cardinality)

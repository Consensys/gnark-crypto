...
contract PlonkVerifier {
  ...
  uint256 private constant G2_SRS_1_X_0_lo = 24738919725606159103622762085301468693996170000835484898627382458668650901798;
	uint256 private constant G2_SRS_1_X_0_hi = 31346352036884794337230464797242014164;
  uint256 private constant G2_SRS_1_X_1_lo = 88988610107324683559751918749328085612251808596667441090329887312656764131137;
  uint256 private constant G2_SRS_1_X_1_hi = 7011016438142167691381309799930879770;
  uint256 private constant G2_SRS_1_Y_0_lo = 82306975308928685175457939384955699569632405398866927857900902072879306241085;
  uint256 private constant G2_SRS_1_Y_0_hi = 9510543664449039842313502326320587369;
  uint256 private constant G2_SRS_1_Y_1_lo = 48656031914361365896317447375940194944434925543387681167324219048167217273255;
  uint256 private constant G2_SRS_1_Y_1_hi = 20641561212720129378295156419001000926;
  ...
  uint256 private constant VK_INV_DOMAIN_SIZE = 50797254075903497026964998617305154405262722734886149140647294365565500522497;
  uint256 private constant VK_OMEGA = 36581797046584068049060372878520385032448812009597153775348195406694427778894;
  uint256 private constant VK_QL_COM_X_lo = 114179222378708944428909958181365955673610897981332352899308048532343750254289;
	uint256 private constant VK_QL_COM_X_hi = 4608102869952627711542235962092322194;
  uint256 private constant VK_QL_COM_Y_lo = 49721760426440938206528982586242629250720064035446242520935025278802449218573;
  uint256 private constant VK_QL_COM_Y_hi = 9994761402783525712196371693657940227;
  uint256 private constant VK_QR_COM_X_lo = 81474049834604797404599613259850234509613569849456411831047391630783669820464;
  uint256 private constant VK_QR_COM_X_hi = 31944902864181706354049416415246106439;
  uint256 private constant VK_QR_COM_Y_lo = 6269774132966614392230221225529692522401530925871597350712063517862504204698;
  uint256 private constant VK_QR_COM_Y_hi = 25646405228149467630587966912256811058;
  uint256 private constant VK_QM_COM_X_lo = 7417865734000065086239552367727687619664051116448856395606173595351871530190;
  uint256 private constant VK_QM_COM_X_hi = 20359766378279654828469052565943820321;
  uint256 private constant VK_QM_COM_Y_lo = 18935055115305016798405509880209570690332307571533611792220473618097230974424;
  uint256 private constant VK_QM_COM_Y_hi = 28975206434380058819195768459767736490;
  uint256 private constant VK_QO_COM_X_lo = 75523757939859414803478049377365512184659847820755468793231869094354902163596;
  uint256 private constant VK_QO_COM_X_hi = 24310501530487618439725694993582890545;
  uint256 private constant VK_QO_COM_Y_lo = 93281695567651715902484261748308943918594899125763006375648368990263406190232;
  uint256 private constant VK_QO_COM_Y_hi = 27225994921181078607480478121808324775;
  uint256 private constant VK_QK_COM_X_lo = 58294453566839569514813234497416695682868452928298924983454164988859401430211;
  uint256 private constant VK_QK_COM_X_hi = 20893356931057690970862372542717003700;
  uint256 private constant VK_QK_COM_Y_lo = 50661952928120582642301169237692589122414300574009078105114770687746177039313;
  uint256 private constant VK_QK_COM_Y_hi = 16863044015946502547894782668553691911;
  
  uint256 private constant VK_S1_COM_X_lo = 84673600974803012569866268245539993445635843775635905532854778504501778783493;
	uint256 private constant VK_S1_COM_X_hi = 31179256942430407185685802863312495986;
  uint256 private constant VK_S1_COM_Y_lo = 33682498952475928668139181001872812981750861167695859001249582345185782996407;
  uint256 private constant VK_S1_COM_Y_hi = 11495599945724277735749984173212140969;
  
  uint256 private constant VK_S2_COM_X_lo = 65665732220415131848377028053599155199696803485165565537153432425101093535877;
	uint256 private constant VK_S2_COM_X_hi = 28861303998307327416810443926252826338;
  uint256 private constant VK_S2_COM_Y_lo = 56112637812907109916429727989605932346974191437332897309177526548775680040264;
  uint256 private constant VK_S2_COM_Y_hi = 12069219964331310634747920018577002552;
  
  uint256 private constant VK_S3_COM_X_lo = 58383323288378710341864551087270626461409023421148455776054989198700688734308;
	uint256 private constant VK_S3_COM_X_hi = 20966049371460800734215202556140037592;
  uint256 private constant VK_S3_COM_Y_lo = 52241471173643661200660593706279167948417570918043227325134309976184349000996;
  uint256 private constant VK_S3_COM_Y_hi = 9623918279334494087043256045575366759;
  
  uint256 private constant VK_COSET_SHIFT = 7;
  
  
  uint256 private constant VK_QCP_0_X_lo = 59752541376058802058496286018574101729515739512657523984592761203883376434957;
	uint256 private constant VK_QCP_0_X_hi = 31262513094459364996842851239014250984;
  uint256 private constant VK_QCP_0_Y_lo = 107483959730501093240507556062062931372597122760022685073962074962548182867996;
  uint256 private constant VK_QCP_0_Y_hi = 30061305731789205758432254989308838463;
  
  uint256 private constant VK_QCP_1_X_lo = 7917269939193731496181582562058795093844906164282272495747081186709171365503;
	uint256 private constant VK_QCP_1_X_hi = 16464266159297004586677213760022481127;
  uint256 private constant VK_QCP_1_Y_lo = 44807862795573009444146001052052289448324631596476754534629986860013370727936;
  uint256 private constant VK_QCP_1_Y_hi = 8132241400789022201829301882790747116;
  
  
  uint256 private constant VK_INDEX_COMMIT_API_0 = 6;
  uint256 private constant VK_INDEX_COMMIT_API_1 = 10;
  ...
  uint256 private constant PROOF_BSB_COMMITMENTS = 0x460;
  ...
  public view returns(bool success) {
    ...
    //   // Beginning challenges -------------------------------------------------
      ...
      function derive_gamma(aproof, nb_pi, pi)->gamma_not_reduced {
        ...
        mstore(add(mPtr, 0x370), VK_QCP_1_X_hi) 
        mstore(add(mPtr, 0x3a0), VK_QCP_1_Y_hi) 
        ...
        mstore(add(mPtr, 0x390), VK_QCP_1_X_lo) 
        mstore(add(mPtr, 0x3c0), VK_QCP_1_Y_lo) 
        
        // public inputs
        let _mPtr := add(mPtr, 0x3e0 )
    ...
    //   // BEGINNING compute_pi -------------------------------------------------
      ...
      function fold_state(aproof) {
        ...
        acc_gamma := mulmod(acc_gamma, l_gamma_kzg, R_MOD)
        fr_acc_mul_calldata(add(state, STATE_FOLDED_CLAIMED_VALUES), add(aproof, add(PROOF_QCP_AT_ZETA, 0x20)), acc_gamma)
        store_point(add(mPtr, 0x460), VK_QCP_1_X_hi, VK_QCP_1_X_lo, VK_QCP_1_Y_hi, VK_QCP_1_Y_lo)
        mstore(add(mPtr, 0x4e0), acc_gamma)
        
        
        let l_success := staticcall(gas(), BLS12_MSM_G1, mPtr, 0x500, add(state, STATE_FOLDED_DIGESTS), 0x80)
        if iszero(l_success){
          error_ec_op()
        }
      ...
      function compute_gamma_kzg(aproof) {
        ...
        mstore(add(mPtr, 0x2d0), VK_QCP_1_X_hi)
        mstore(add(mPtr, 0x300), VK_QCP_1_Y_hi)
        ...
        mstore(add(mPtr, 0x2f0), VK_QCP_1_X_lo)
        mstore(add(mPtr, 0x320), VK_QCP_1_Y_lo)
        ...
        mstore(add(mPtr, 0x340), mload(add(state, STATE_OPENING_LINEARISED_POLYNOMIAL_ZETA))) 
        mstore(add(mPtr, 0x360), calldataload(add(aproof, PROOF_L_AT_ZETA)))
        mstore(add(mPtr, 0x380), calldataload(add(aproof, PROOF_R_AT_ZETA)))
        mstore(add(mPtr, 0x3a0), calldataload(add(aproof, PROOF_O_AT_ZETA)))
        mstore(add(mPtr, 0x3c0), calldataload(add(aproof, PROOF_S1_AT_ZETA)))
        mstore(add(mPtr, 0x3e0), calldataload(add(aproof, PROOF_S2_AT_ZETA)))
        let _poqaz := add(aproof, PROOF_QCP_AT_ZETA)
        calldatacopy(add(mPtr, 0x400), _poqaz, mul(VK_NB_CUSTOM_GATES, 0x20))
        
        mstore(add(mPtr, 0x440), calldataload(add(aproof, PROOF_GRAND_PRODUCT_AT_ZETA_OMEGA)))

        let start_input := 0x1b // 00.."gamma"
        
        let check_staticcall := staticcall(gas(), SHA2, add(mPtr,start_input), 0x445, add(state, STATE_GAMMA_KZG), 0x20)
      ...
      function compute_commitment_linearised_polynomial_ec(aproof, s1, coeff_z) {
        ...
        store_point_calldata(add(mPtr, 0x6e0), add(aproof, add(PROOF_BSB_COMMITMENTS, 0x60)))
        ...
        calldatacopy(add(mPtr, 0x760), add(aproof, add(PROOF_QCP_AT_ZETA, 0x20)), 0x20)
        
        let l_success := staticcall(gas(), BLS12_MSM_G1, mPtr, 0x780, add(state, STATE_LINEARISED_POLYNOMIAL), 0x80)
        if iszero(l_success){
          error_ec_op()
        }

...
func TestE6DReceiverIsOperand(t *testing.T) {
	...
	properties.Property("[(direct) koalabear] conversion between direct and tower extensions", prop.ForAll(
		func(a *E6D) bool {
			b := ToTower(a)
			c := FromTower(b)
			return a.Equal(c)
	...
	properties.Property("[(direct) koalabear] Having the receiver as operand (Inverse) should output the same result", prop.ForAll(
		func(a *E6D) bool {
			var b E6D
			b.Inverse(a)
			a.Inverse(a)
	...
	properties.Property("[(direct) koalabear] Having the receiver as operand (Conjugate) should output the same result", prop.ForAll(
		func(a *E6D) bool {
			var b E6D
			b.Conjugate(a)
			a.Conjugate(a)
...
func TestE6DOps(t *testing.T) {
	...
	properties.Property("[(direct) koalabear] mul & inverse should leave an element invariant", prop.ForAll(
		func(a, b *E6D) bool {
			var c, d E6D
			d.Inverse(b)
			c.Set(a)
	...
	properties.Property("[(direct) koalabear] inverse twice should leave an element invariant", prop.ForAll(
		func(a *E6D) bool {
			var b E6D
	...
	properties.Property("[(direct) koalabear] square and mul should output the same result", prop.ForAll(
		func(a *E6D) bool {
			var b, c E6D
			b.Mul(a, a)
			c.Square(a)

	...
	if opt.coset {
		if decimation == DIT {
			...
			if domain.withPrecompute {
				parallel.Execute(len(a), func(start, end int) {
					v1 := koalabear.Vector(a[start:end])
					v2 := koalabear.Vector(domain.cosetTable[start:end])
					v1.Mul(v1, v2)
				}, opt.nbTasks)
			} else {
...
func innerDIFWithTwiddlesGeneric(a []koalabear.Element, twiddles []koalabear.Element, start, end, m int) {
	koalabear.Vector(a[start:end]).Butterfly(koalabear.Vector(a[start+m : end+m]))
	if start == 0 {
		start++
	}
...
func innerDITWithTwiddlesGeneric(a []koalabear.Element, twiddles []koalabear.Element, start, end, m int) {
	...
	if twStart < end {
		v1 := koalabear.Vector(a[twStart+m : end+m])
		v2 := koalabear.Vector(twiddles[twStart:end])
		v1.Mul(v1, v2)
	}
	koalabear.Vector(a[start:end]).Butterfly(koalabear.Vector(a[start+m : end+m]))
}
...
func innerDITWithoutTwiddles(a []koalabear.Element, at, w koalabear.Element, start, end, m int) {
	...
	if twStart < end {
		// precompute twiddles for this chunk
		tw := make([]koalabear.Element, end-twStart)
		tw[0].Set(&at)
		for i := 1; i < len(tw); i++ {
			tw[i].Mul(&tw[i-1], &w)
		}

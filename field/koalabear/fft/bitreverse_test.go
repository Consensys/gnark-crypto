...
var koalabearBitReverse = []bitReverseVariant[koalabear.Element]{
	{name: "bitReverseNaive", buf: make([]koalabear.Element, maxSizeBitReverse), fn: bitReverseNaive[koalabear.Element]},
	{name: "BitReverse", buf: make([]koalabear.Element, maxSizeBitReverse), fn: BitReverse[koalabear.Element]},
}


func TestKoalabearBitReverse(t *testing.T) {
	...
	for size := 2; size <= maxSizeBitReverse; size <<= 1 {
		...
		for _, data := range koalabearBitReverse {
		...
		for i := 0; i < size; i++ {
			for j := 1; j < len(koalabearBitReverse); j++ {
				if !koalabearBitReverse[0].buf[i].Equal(&koalabearBitReverse[j].buf[i]) {
					t.Fatalf("bitReverse %s and %s do not compute the same result", koalabearBitReverse[0].name, koalabearBitReverse[j].name)
				}
			}
		}
		...
		for i := 0; i < size; i++ {
			for j := 1; j < len(koalabearBitReverse); j++ {
				if !koalabearBitReverse[0].buf[i].Equal(&koalabearBitReverse[j].buf[i]) {
					t.Fatalf("(fn-1) bitReverse %s and %s do not compute the same result", koalabearBitReverse[0].name, koalabearBitReverse[j].name)
				}
			}
		}
	}
...
func BenchmarkKoalabearBitReverse(b *testing.B) {

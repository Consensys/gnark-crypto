...
// returns a koalabear.Element
func (p *Polynomial) Eval(v *koalabear.Element) koalabear.Element {
...
func (p *Polynomial) AddConstantInPlace(c *koalabear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Add(&(*p)[i], c)
	}
}
...
func (p *Polynomial) SubConstantInPlace(c *koalabear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Sub(&(*p)[i], c)
	}
}
...
func (p *Polynomial) ScaleInPlace(c *koalabear.Element) {
	for i := 0; i < len(*p); i++ {
		(*p)[i].Mul(&(*p)[i], c)
	}
}
...
func (p *Polynomial) Scale(c *koalabear.Element, p0 Polynomial) {
	if len(*p) != len(p0) {
...
func InterpolateOnRange(v []koalabear.Element) Polynomial {
	nEvals := uint8(len(v))
	if int(nEvals) != len(v) {
...
func computeLagrangeBasis(domainSize uint8) []Polynomial {
	...
	constTerms := make([]koalabear.Element, domainSize)
	...
	constTerms = koalabear.BatchInvert(constTerms)
	for l := uint8(0); l < domainSize; l++ {

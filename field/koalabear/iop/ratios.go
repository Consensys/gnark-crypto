...
func BuildRatioShuffledVectors(numerator, denominator []*Polynomial, beta koalabear.Element, expectedForm Form, domain *fft.Domain) (*Polynomial, error) {
...
func BuildRatioCopyConstraint(
	...
	parallel.Execute(n-1, func(start, end int) {
		...
		tInv := koalabear.BatchInvert(t[start:end])
		for i := start; i < end; i++ {
...
func getSupportIdentityPermutation(nbCopies int, domain *fft.Domain) []koalabear.Element {
	if nbCopies <= 0 {
		panic("getSupportIdentityPermutation: nbCopies must be positive")
	}

	res := make([]koalabear.Element, uint64(nbCopies)*domain.Cardinality)

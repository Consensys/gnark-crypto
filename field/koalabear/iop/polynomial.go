...
type Polynomial struct {
	*polynomial
	shift int
	size  int
	coset koalabear.Element // needed for evaluating the polynomial when it is expressed in Lagrange shifted basis
}
...
func NewPolynomial(coeffs *[]koalabear.Element, form Form) *Polynomial {
	return &Polynomial{
...
func (p *Polynomial) Evaluate(x koalabear.Element) koalabear.Element {
...
func (p *Polynomial) GetCoeff(i int) koalabear.Element {
...
type polynomial struct {
	coefficients *koalabear.Vector
	Form
}

// Coefficients returns a slice on the underlying data structure.
func (p *polynomial) Coefficients() []koalabear.Element {
...
func newPolynomial(coeffs *[]koalabear.Element, form Form) *polynomial {
	return &polynomial{coefficients: (*koalabear.Vector)(coeffs), Form: form}
}
...
func (p *polynomial) clone(capacity ...int) *polynomial {
	...
	newCoeffs := make(koalabear.Vector, p.coefficients.Len(), c)
	r := &polynomial{
...
func (p *polynomial) evaluate(x koalabear.Element) koalabear.Element {
	...
	evalLagrange := func() {
		...
		var accw koalabear.Element
		accw.SetOne()
		dens := make([]koalabear.Element, sizeP) // [x-1, x-ω, x-ω², ...]
		for i := 0; i < sizeP; i++ {
			dens[i].Sub(&x, &accw)
			accw.Mul(&accw, &w)
		}
		invdens := koalabear.BatchInvert(dens) // [1/(x-1), 1/(x-ω), 1/(x-ω²), ...]
		var tmp koalabear.Element
		var one koalabear.Element
		one.SetOne()
		tmp.Exp(x, big.NewInt(int64(sizeP))).Sub(&tmp, &one) // xⁿ-1
		var li koalabear.Element
...
func (p *polynomial) grow(newSize int) {
	...
	if offset > 0 {
		(*p.coefficients) = append((*p.coefficients), make(koalabear.Vector, offset)...)
	}
}
...
func (p *Polynomial) WriteTo(w io.Writer) (int64, error) {
	...
	koalabear.BigEndian.PutElement(&buf, p.coset)
	m, err := w.Write(buf[:])
	n += int64(m)
	if err != nil {
		return n, err
...
func (p *Polynomial) ReadFrom(r io.Reader) (int64, error) {
	...
	var bufFr [koalabear.Bytes]byte
	read, err := io.ReadFull(r, bufFr[:])
	n += int64(read)
	if err != nil {
		return n, err
	}
	p.coset, err = koalabear.BigEndian.Element(&bufFr)
	if err != nil {
		return n, err
	}

...
type RSis struct {
	...
	smallFFT   func(koalabear.Vector)
	cosetTable []koalabear.Element // used in conjunction with the smallFFT;
}
...
func NewRSis(seed int64, logTwoDegree, logTwoBound, maxNbElementsToHash int) (*RSis, error) {
	...
	if r.Domain.Cardinality == 64 {
		...
		r.smallFFT = func(p koalabear.Vector) {
			fft64(p, twiddlesCoset)
		}
	} else {
		r.smallFFT = func(p koalabear.Vector) {
			p.Mul(p, koalabear.Vector(r.cosetTable))

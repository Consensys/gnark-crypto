...
TEXT ·mulAssignElement(SB), NOSPLIT, $0-16
    ...
    ADCXQ DX, R9
    ADOXQ DX, R9
    MOVQ $0xfffffffeffffffff, DX
    MULXQ CX, R8, DX                                        // m := t[0]*q'[0] mod W
    XORQ DX, DX                                            // clear the flags
    // C,_ := t[0] + m*q[0]
    MOVQ $0xffffffff00000001, DX
    MULXQ R8, AX, DX
    ADCXQ CX, AX
    MOVQ DX, CX
    // for j=1 to N-1
    //     (C,t[j-1]) := t[j] + m*q[j] + C
    MOVQ $0x53bda402fffe5bfe, DX
    ADCXQ BX, CX
    MULXQ R8, AX, BX
    ADOXQ AX, CX
    MOVQ $0x3339d80809a1d805, DX
    ADCXQ BP, BX
    MULXQ R8, AX, BP
    ADOXQ AX, BX
    MOVQ $0x73eda753299d7d48, DX
    ADCXQ SI, BP
    MULXQ R8, AX, SI
    ADOXQ AX, BP

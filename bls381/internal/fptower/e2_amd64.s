...
TEXT ·innerMulAdxE2(SB), NOSPLIT, $0-32
    MOVQ x+0(FP), AX
    MOVQ 0(AX), R14
    MOVQ 8(AX), R15
    MOVQ 16(AX), CX
    ...
    //     (C,t[j-1]) := t[j] + m*q[j] + C
    ADCXQ R13, R12
    MULXQ ·qE2+8(SB), AX, R13
    ADOXQ AX, R12
    ADCXQ DI, R13
    MULXQ ·qE2+16(SB), AX, DI
    ADOXQ AX, R13
    ADCXQ R8, DI
    MULXQ ·qE2+24(SB), AX, R8
    ADOXQ AX, DI
    ADCXQ R9, R8
    MULXQ ·qE2+32(SB), AX, R9
    ADOXQ AX, R8
    ADCXQ R10, R9
    MULXQ ·qE2+40(SB), AX, R10
    ADOXQ AX, R9
    MOVQ $0x0000000000000000, AX
    ADCXQ AX, R10
    ADOXQ R11, R10

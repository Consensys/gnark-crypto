...
// lllReduceWithBound performs LLL reduction with early termination.
// It stops early if it finds a row where:
// - At least one of the denominator columns (denCols) is non-zero
// - All components have absolute value <= bound
// Returns the index of the satisfying row, or -1 if none found during early termination.
func lllReduceWithBound(basis [][]big.Int, m int, bound *big.Int, denCols []int) int {
	...
	// Helper to check if a row satisfies the bound condition
	...
	// Check initial rows before starting LLL
	for i := 0; i < m; i++ {
		if checkRow(i) {
			return i
		}

...
func TestCubicalPairingFixedQMultipleP(t *testing.T) {
	...
	// Compute using PairCubicalFixedQMulti (batched)
	batchedResult, err := PairCubicalFixedQMulti(P, pre)
	if err != nil {
		t.Fatalf("Batched fixed-Q pairing failed: %v", err)
	}
	...
	// Compare
	if !batchedResult.Equal(&standardResult) {
		t.Errorf("Batched fixed-Q multi-P result does not match standard cubical pairing")
	}
}

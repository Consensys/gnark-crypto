...
func (p *PointAffine) IsInSubGroup() bool {
	...
	// (x_m, y_m) is a point on the birationally equivalent Montgomery curve M,
	// where:
	// 		x_m = (1-y_e)/(1+y_e)
	// 		y_m = x_m/x_e
	//
	// We need to check that the tate pairing t_{8,Q8}(P) is 1 (the other
	// pairing t_1 is trivial, i.e. 1) where Q8 is a point of order 8 on the
	// Montgomery curve M.
	//
	// The point Q8:
	// (0x6c02088449a5fb6190457206870a13ca2416b6718b70bba289be291b1dcd0d32,
	// 0x2d170bfdc1c4196633553658379ed70d6723dc103b62e60eb885e7ae2024963a)
	// is of order 8 and the associated Miller function is:
	// 		f_{8,Q8} = (l0/v1)^4 * l1^2/v2
	// where l0, l1, v1, v2 are pre-computed lines of the form: y+ax+b=0.
	// Explicitly,
	// 		l0 = y_m + x1*x_m + x2
	// 		l1 = y_m + t0*x_m
	// 		v1 = x_m + b
	// 		v2 = x_m
	// with x1,x2,t0,b precomputed constants.
	//
	// To avoid inverses we use the fact that ((a/b) / r)_2 = (a * b / r)_2.
	// Thus we compute: f_{8,Q8} = (l0*v1)^4 * l1^2 * v2
	//
	// Replacing x_m and y_m by their expressions in terms of x_e and y_e,
	// we get:
	// 		f_{8,Q8} = num * denom
	// where:
	// 		num   = (((1 + y)(1 + x1*x) + x2*x(1 - y))^4) * (((1 + b) + (1 - b)y)^4) * ((1 + y)^3) * ((1 + t0*x)^2)
	// 		denom = (1 - y)^11 * x^6

	var (
		one, tmp, res          fr.Element
		numerator, denominator fr.Element
		onePlusY, oneMinusY    fr.Element
		pow2, pow4, pow8       fr.Element
	)
	one.SetInt64(1)

	onePlusY.Add(&one, &p.Y)
	tmp.Mul(&curveParams.x1, &p.X)
	res.Add(&one, &tmp)
	tmp.Mul(&onePlusY, &res)
	oneMinusY.Sub(&one, &p.Y)
	res.Mul(&curveParams.x2, &p.X)
	res.Mul(&res, &oneMinusY)
	tmp.Add(&tmp, &res)
	numerator.Square(&tmp)
	numerator.Square(&numerator)
	res.Add(&one, &curveParams.b)
	tmp.Sub(&one, &curveParams.b)
	tmp.Mul(&tmp, &p.Y)
	res.Add(&res, &tmp)
	tmp.Square(&res)
	tmp.Square(&tmp)
	...
	tmp.Square(&onePlusY)
	tmp.Mul(&tmp, &onePlusY)
	numerator.Mul(&numerator, &tmp)
	res.Mul(&curveParams.t0, &p.X)
	res.Add(&one, &res)
	tmp.Square(&res)
	numerator.Mul(&numerator, &tmp)
	tmp.Square(&p.X)
	res.Mul(&p.X, &tmp)
	denominator.Square(&res)
	pow2.Square(&oneMinusY)
	pow4.Square(&pow2)
	pow8.Square(&pow4)
	tmp.Mul(&pow8, &pow2)
	tmp.Mul(&tmp, &oneMinusY)
	denominator.Mul(&denominator, &tmp)
	res.Mul(&numerator, &denominator)

	return res.Legendre() == 1
}

...
func (z *Element) legendreP20() int {
	...
	// Therefore, we use the properties of the Kronecker symbol, which generalizes the Legendre symbol to all integers.
	...
	for !a.IsZero() {
		...
		for range nbIterationsInnerLoop { // calling the loop variable j in the comments below
			...
			// |f₀| + |f₁| < 2ʲ⁺¹
			if aApprox&1 == 0 {
				// TODO try batching these together (since b doesn't change) and advancing the loop variable j manually
				// so that we don't "decide" over and over again whether to flip l
				aApprox /= 2
				...
				if borrow == 1 {
					...
					// Again since not both a and b are negative, we get a quadratic reciprocity law
		...
		if aHi&signBitSelector != 0 {
			// if aHi < 0
			aHi = negL(&a, aHi)
			// since a is negative, b is not and hence b ≠ -1 TODO IS B REALLY NOT NEGATIVE
	...
	if b[0] == 1 && (b[1]|b[2]|b[3]|b[4]|b[5]) == 0 {
		...
		return 0 // if b ≠ 1, then (x,q) ≠ 0 ⇒ (x|q) = 0
	}
}

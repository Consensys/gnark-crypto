...
// Optimized: Implements the constant-time Kronecker/Legendre symbol algorithm from
//   Diego F. Aranha et al, "Faster constant-time evaluation of the Kronecker symbol with application to elliptic curve hashing"
//   https://eprint.iacr.org/2023/1261
// This is a word-based, constant-time algorithm generalizing Bernstein-Yang's modular inverse.
func (z *Element) Legendre() int {
	...
	// Copy input and modulus
	a := *z
	b := qElement

	// l tracks the accumulated sign
	l := 1

	for {
		// Remove factors of 2 from a
		for a[0]&1 == 0 && !a.IsZero() {
			// (2|b): 1 if b ≡ 1,7 mod 8; -1 if b ≡ 3,5 mod 8
			bMod8 := b[0] & 7
			if bMod8 == 3 || bMod8 == 5 {
				l = -l
			}

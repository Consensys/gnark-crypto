...
func ToLagrangeG1(coeffs []G1Affine) error {
...
func butterflyG1(a *G1Affine, b *G1Affine) {
	t := *a
	a.Add(a, b)
	b.Sub(&t, b)
}
...
func kerDIF8G1(a []G1Affine, twiddles [][]fr.Element, stage int) {
	butterflyG1(&a[0], &a[4])
	butterflyG1(&a[1], &a[5])
	butterflyG1(&a[2], &a[6])
	butterflyG1(&a[3], &a[7])
...
func difFFTG1(a []G1Affine, twiddles [][]fr.Element, stage, maxSplits int, chDone chan struct{}) {
	if chDone != nil {
		defer close(chDone)
	}

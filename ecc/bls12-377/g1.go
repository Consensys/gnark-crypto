...
func mulByTau(x, y fp.Element) (fp.Element, fp.Element) {
	// Xτ = 4Y² − 3X³
	// Yτ = Y(9X³ − 8Y²)
...
func (p *G1Jac) Triple(q *G1Jac) *G1Jac {
	...
	// Xτ = 4Y² − 3X³
	// Yτ = Y(9X³ − 8Y²)
	xτ, yτ = mulByTau(q.X, q.Y)

	// X3 = 4Yτ² − 3Xτ³
	// Y3 = Yτ(9Xτ³ − 8Yτ²)
	p.X, p.Y = mulByTau(xτ, yτ)
...
// 	// a, b := mulByTau(q.X, q.Y)
// 	// fmt.Println(xτ.String())
// 	// fmt.Println(a.String())
// 	// fmt.Println(yτ.String())
...
func (p *G1Jac) mulBySeed(q *G1Jac) *G1Jac {
	...
	// Operations: 59 doublings, 2 triplings and 5 additions.

...
func innerDIFWithTwiddlesGeneric(a []fr.Element, twiddles []fr.Element, start, end, m int) {
	fr.Vector(a[start:end]).Butterfly(fr.Vector(a[start+m : end+m]))
	if start == 0 {
		start++
	}
...
func innerDITWithTwiddlesGeneric(a []fr.Element, twiddles []fr.Element, start, end, m int) {
	...
	if twStart < end {
		v1 := fr.Vector(a[twStart+m : end+m])
		v2 := fr.Vector(twiddles[twStart:end])
		v1.Mul(v1, v2)
	}
	fr.Vector(a[start:end]).Butterfly(fr.Vector(a[start+m : end+m]))
}
...
func innerDITWithoutTwiddles(a []fr.Element, at, w fr.Element, start, end, m int) {
	...
	if twStart < end {
		// precompute twiddles for this chunk
		tw := make([]fr.Element, end-twStart)
		tw[0].Set(&at)
		for i := 1; i < len(tw); i++ {
			tw[i].Mul(&tw[i-1], &w)
		}

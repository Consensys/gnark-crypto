...
// In an interactive setting, it must be randomized and sent over by the verifier.
...
// combinationCoeff is used as a linear combination coefficient to fold separate commitments into one.
// In an interactive setting, it must be randomized and sent over by the verifier.
// Otherwise, it must be generated via Fiat-Shamir.
...
func BatchVerifyMultiVk(vk []VerifyingKey, commitments []curve.G1Affine, pok []curve.G1Affine, combinationCoeff fr.Element) error {
	...
	if commitment, err := FoldCommitments(commitments, combinationCoeff); err != nil {
		return err
	} else {

...
// the birational map χ₂,₃ from https://eprint.iacr.org/2021/1446.pdf (Section 3).
// Decompression requires only ONE cube root extraction instead of two square roots.
// Note: For BW6-761, G2 is also defined over Fp (not Fp2), so this uses fp.Element.
...
// extraction (instead of two square roots). Based on https://eprint.iacr.org/2021/1446.pdf
// Note: For BW6-761, G2 is also defined over Fp (not Fp2).
...
const (
	...
	SizeOfBatchCompressedG1Pair           = fp.Bytes + fp.Bytes     // 192 bytes (generic)
	SizeOfBatchCompressedG1PairDegenerate = fp.Bytes + fp.Bytes + 1 // 193 bytes (degenerate)

	// SizeOfBatchCompressedG2Pair is the size of two G2 points batch-compressed.
	// Generic case: 192 bytes (z0 + z1, n encoded in z0's high bits)
	// Degenerate case: 193 bytes (z0 + z1 + flags byte)
	SizeOfBatchCompressedG2Pair           = fp.Bytes + fp.Bytes     // 192 bytes (generic)
	SizeOfBatchCompressedG2PairDegenerate = fp.Bytes + fp.Bytes + 1 // 193 bytes (degenerate)
)
...
func BatchCompressG1Slice(points []G1Affine) ([]byte, error) {
	...
	parallel.Execute(nPairs, func(start, end int) {
		for i := start; i < end; i++ {
			...
			if isGeneric {
				pairSizes[i] = SizeOfBatchCompressedG1Pair // 192 bytes
			} else {
				pairSizes[i] = SizeOfBatchCompressedG1PairDegenerate // 193 bytes
			}

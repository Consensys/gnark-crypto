...
func (privKey *PrivateKey) Sign(message []byte, hFunc hash.Hash) ([]byte, error) {
	...
	dst := []byte("0x01")
	H, err := bls24317.HashToG2(message, dst)
	if err != nil {
		return nil, err
	}
...
func (publicKey *PublicKey) Verify(sigBin, message []byte, hFunc hash.Hash) (bool, error) {
	...
	_, _, g1, _ := bls24317.Generators()
	g1.Neg(&g1)
	f, err := bls24317.PairingCheck([]bls24317.G1Affine{g1, publicKey.A}, []bls24317.G2Affine{sig.S, H})
	if err != nil {
		return false, err
	}

...
func RegisterGate(name string, gate Gate) {
	gates.Store(name, gate)
}

func GetGate(name string) Gate {
	gate, found := gates.Load(name)
	if !found {
		return nil
	}
	return gate.(Gate)
...
func init() {
	RegisterGate("identity", IdentityGate{})
	RegisterGate("add", AddGate{})
	RegisterGate("sub", SubGate{})
	RegisterGate("neg", NegGate{})
	RegisterGate("mul", MulGate(2))
}

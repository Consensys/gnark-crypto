...
func randomOnG1() (bw6756.G1Affine, error) { // TODO: Add to G1.go?
	gBytes := make([]byte, fr.Bytes)
	if _, err := rand.Read(gBytes); err != nil {
		return bw6756.G1Affine{}, err
	}
	return bw6756.HashToG1(gBytes, []byte("random on g2"))
}

func testCommit(t *testing.T, values ...interface{}) {

	basis := make([]bw6756.G1Affine, len(values))
	for i := range basis {
		var err error
	...
	var (
		key             Key
		err             error
		commitment, pok bw6756.G1Affine
	)

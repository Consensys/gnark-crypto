...
import (
	...
	"github.com/consensys/gnark-crypto/ecc/bls12-377-strong"
...
type Digest = bls12377strong.G1Affine

// ProvingKey used to create or open commitments
type ProvingKey struct {
	G1 []bls12377strong.G1Affine // [G₁ [α]G₁ , [α²]G₁, ... ]
}

// VerifyingKey used to verify opening proofs
type VerifyingKey struct {
	G2    [2]bls12377strong.G2Affine // [G₂, [α]G₂ ]
	G1    bls12377strong.G1Affine
	Lines [2][2][len(bls12377strong.LoopCounter) - 1]bls12377strong.LineEvaluationAff // precomputed pairing lines corresponding to G₂, [α]G₂
}
...
func NewSRS(size uint64, bAlpha *big.Int) (*SRS, error) {
	...
	var srs SRS
	srs.Pk.G1 = make([]bls12377strong.G1Affine, size)
	...
	_, _, gen1Aff, gen2Aff := bls12377strong.Generators()
	...
	if bAlpha.Cmp(&bMOne) == 0 {
		...
		var g [4]bls12377strong.G1Affine
		g[0] = gen1Aff
		for i := 1; i < 4; i++ {
		...
		srs.Vk.Lines[0] = bls12377strong.PrecomputeLines(srs.Vk.G2[0])
		srs.Vk.Lines[1] = bls12377strong.PrecomputeLines(srs.Vk.G2[1])
		return &srs, nil
	}
	srs.Pk.G1[0] = gen1Aff
	srs.Vk.G1 = gen1Aff
	...
	g1s := bls12377strong.BatchScalarMultiplicationG1(&gen1Aff, alphas)
...
func Commit(p []fr.Element, pk ProvingKey, nbTasks ...int) (Digest, error) {
	...
	var res bls12377strong.G1Affine
...
func Verify(commitment *Digest, proof *OpeningProof, point fr.Element, vk VerifyingKey) error {
	...
	var totalG1 bls12377strong.G1Jac
	...
	var commitmentJac bls12377strong.G1Jac
	commitmentJac.FromAffine(commitment)
	totalG1.SubAssign(&commitmentJac)

	// e([f(α)-f(a)+aH(α)]G₁], G₂).e([-H(α)]G₁, [α]G₂) == 1
	var totalG1Aff bls12377strong.G1Affine
	totalG1Aff.FromJacobian(&totalG1)
	check, err := bls12377strong.PairingCheckFixedQ(
		[]bls12377strong.G1Affine{totalG1Aff, proof.H},
		vk.Lines[:],
	)
...
func BatchVerifyMultiPoints(digests []Digest, proofs []OpeningProof, points []fr.Element, vk VerifyingKey) error {
	...
	var foldedQuotients bls12377strong.G1Affine
	quotients := make([]bls12377strong.G1Affine, len(proofs))
	...
	var foldedEvalsCommit bls12377strong.G1Affine
	...
	// ∑ᵢλᵢ[p_i]([Hᵢ(α)]G₁)
	var foldedPointsQuotients bls12377strong.G1Affine
	for i := 0; i < len(randomNumbers); i++ {
	...
	check, err := bls12377strong.PairingCheckFixedQ(
		[]bls12377strong.G1Affine{foldedDigests, foldedQuotients},
		vk.Lines[:],
	)
	if err != nil {
		return err

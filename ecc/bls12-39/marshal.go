...
// Encoder writes bls12-39 object values to an output stream
type Encoder struct {
	w   io.Writer
...
// Decoder reads bls12-39 object values from an inbound stream
type Decoder struct {
	r             io.Reader
	n             int64 // read bytes
...
// NewDecoder returns a binary decoder supporting curve bls12-39 objects in both
...
func (dec *Decoder) Decode(v interface{}) (err error) {
	...
	if rv.Kind() != reflect.Ptr || rv.IsNil() || !rv.Elem().CanSet() {
		return errors.New("bls12-39 decoder: unsupported type, need pointer")
	}
	...
	default:
		n := binary.Size(t)
		if n == -1 {
			return errors.New("bls12-39 encoder: unsupported type")
		}
...
// NewEncoder returns a binary encoder supporting curve bls12-39 objects
...
const SizeOfG1AffineCompressed = 8
...
func (p *G1Affine) Bytes() (res [SizeOfG1AffineCompressed]byte) {
	...
	tmp = p.X
	tmp.FromMont()
	binary.BigEndian.PutUint64(res[0:8], tmp[0])
...
func (p *G1Affine) RawBytes() (res [SizeOfG1AffineUncompressed]byte) {
	...
	tmp = p.Y
	tmp.FromMont()
	binary.BigEndian.PutUint64(res[8:16], tmp[0])
...
const SizeOfG2AffineCompressed = 8 * 2
...
func (p *G2Affine) RawBytes() (res [SizeOfG2AffineUncompressed]byte) {
	...
	tmp = p.Y.A1
	tmp.FromMont()
	binary.BigEndian.PutUint64(res[16:24], tmp[0])

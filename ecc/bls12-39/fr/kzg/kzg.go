...
import (
	...
	"github.com/consensys/gnark-crypto/ecc/bls12-39"
...
type Digest = bls1239.G1Affine

// SRS stores the result of the MPC
type SRS struct {
	G1 []bls1239.G1Affine  // [G₁ [α]G₁ , [α²]G₁, ... ]
	G2 [2]bls1239.G2Affine // [G₂, [α]G₂ ]
}
...
func NewSRS(size uint64, bAlpha *big.Int) (*SRS, error) {
	...
	var srs SRS
	srs.G1 = make([]bls1239.G1Affine, size)

	var alpha fr.Element
	alpha.SetBigInt(bAlpha)

	_, _, gen1Aff, gen2Aff := bls1239.Generators()
	srs.G1[0] = gen1Aff
	srs.G2[0] = gen2Aff
	...
	g1s := bls1239.BatchScalarMultiplicationG1(&gen1Aff, alphas)
	copy(srs.G1[1:], g1s)
...
func Verify(commitment *Digest, proof *OpeningProof, point fr.Element, srs *SRS) error {
	...
	// [f(a)]G₁
	var claimedValueG1Aff bls1239.G1Affine
	...
	// [f(α) - f(a)]G₁
	var fminusfaG1Jac, tmpG1Jac bls1239.G1Jac
	fminusfaG1Jac.FromAffine(commitment)
	tmpG1Jac.FromAffine(&claimedValueG1Aff)
	fminusfaG1Jac.SubAssign(&tmpG1Jac)

	// [-H(α)]G₁
	var negH bls1239.G1Affine
	negH.Neg(&proof.H)

	// [α-a]G₂
	var alphaMinusaG2Jac, genG2Jac, alphaG2Jac bls1239.G2Jac
	...
	// [α-a]G₂
	var xminusaG2Aff bls1239.G2Affine
	xminusaG2Aff.FromJacobian(&alphaMinusaG2Jac)

	// [f(α) - f(a)]G₁
	var fminusfaG1Aff bls1239.G1Affine
	fminusfaG1Aff.FromJacobian(&fminusfaG1Jac)

	// e([f(α) - f(a)]G₁, G₂).e([-H(α)]G₁, [α-a]G₂) ==? 1
	check, err := bls1239.PairingCheck(
		[]bls1239.G1Affine{fminusfaG1Aff, negH},
		[]bls1239.G2Affine{srs.G2[0], xminusaG2Aff},
	)
	if err != nil {
		return err
	}
...
func BatchVerifyMultiPoints(digests []Digest, proofs []OpeningProof, points []fr.Element, srs *SRS) error {
	...
	var foldedQuotients bls1239.G1Affine
	quotients := make([]bls1239.G1Affine, len(proofs))
	...
	var foldedEvalsCommit bls1239.G1Affine
	...
	// ∑ᵢλᵢ[p_i]([Hᵢ(α)]G₁)
	var foldedPointsQuotients bls1239.G1Affine
	for i := 0; i < len(randomNumbers); i++ {
	...
	check, err := bls1239.PairingCheck(
		[]bls1239.G1Affine{foldedDigests, foldedQuotients},
		[]bls1239.G2Affine{srs.G2[0], srs.G2[1]},
	)
	if err != nil {
		return err
	}

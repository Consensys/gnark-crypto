...
import (
	...
	bw6633_pol "github.com/consensys/gnark-crypto/ecc/bw6-633/fr/polynomial"
...
type Scheme struct {
	...
	SRS struct {
		G1 []bw6633.G1Affine  // [gen [alpha]gen , [alpha**2]gen, ... ]
		G2 [2]bw6633.G2Affine // [gen, [alpha]gen ]
	}
}
...
func NewScheme(size int, alpha fr.Element) *Scheme {
	...
	s.Domain = *d
	s.SRS.G1 = make([]bw6633.G1Affine, size)
...
func (s *Scheme) Commit(p polynomial.Polynomial) (polynomial.Digest, error) {
	...
	var res Digest
	_p := p.(*bw6633_pol.Polynomial)

	// ensure we don't modify p
	pCopy := make(bw6633_pol.Polynomial, s.Domain.Cardinality)
	copy(pCopy, *_p)
...
func (s *Scheme) Verify(commitment polynomial.Digest, proof polynomial.OpeningProof) error {
	...
	_commitment := commitment.(*bw6633.G1Affine)
	_proof := proof.(*Proof)
	...
	check, err := bw6633.PairingCheck(
		...
		[]bw6633.G2Affine{s.SRS.G2[0], xminusaG2Aff},
	)
	if err != nil {
		return err
	}
...
func (s *Scheme) BatchOpenSinglePoint(point interface{}, digests []polynomial.Digest, polynomials []polynomial.Polynomial) (polynomial.BatchOpeningProofSinglePoint, error) {
	...
	// compute H
	_sumGammaiTimesPol := sumGammaiTimesPol.(*bw6633_pol.Polynomial)
...
func (s *Scheme) BatchVerifySinglePoint(digests []polynomial.Digest, batchOpeningProof polynomial.BatchOpeningProofSinglePoint) error {
	...
	subgroupCheck := true
	for i := 0; i < len(digests); i++ {
		_digest := digests[i].(*bw6633.G1Affine)
	...
	for i := 0; i < len(digests); i++ {
		_digests[i].Set(digests[i].(*bw6633.G1Affine))
	}

...
func getSupportIdentityPermutation(nbCopies int, domain *fft.Domain) []fr.Element {
	...
	// check if we can reuse the pre-computed twiddles from the domain.
	if len(domain.Twiddles) == 0 || len(domain.Twiddles[0]) < sizePoly/2 {
		res[0].SetOne()
		if len(res) > 1 {
		...
		// re-use pre-computed twiddles from the domain.

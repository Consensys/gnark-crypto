...
func TestE6ExptGroundTruth(t *testing.T) {
	t.Parallel()
...
func TestE6ExptMinus1SquaredGroundTruth(t *testing.T) {
	t.Parallel()
	e := new(big.Int).SetUint64(10356037238743105536)
	for i := 0; i < 20; i++ {
		a := randomCyclotomicE6()
		var expect, got E6
		expect.CyclotomicExp(a, e)
		got.ExptMinus1Squared(&a)
		if !expect.Equal(&got) {
			t.Fatal("ExptMinus1Squared does not match generic exponentiation by (t-1)^2")
		}
	}
}

func TestE6ExptSquarePlus1GroundTruth(t *testing.T) {
	t.Parallel()
	e := new(big.Int).SetUint64(10356037232306946050)
	for i := 0; i < 20; i++ {
		a := randomCyclotomicE6()
		var expect, got E6
		expect.CyclotomicExp(a, e)
		got.ExptSquarePlus1(&a)
		if !expect.Equal(&got) {
			t.Fatal("ExptSquarePlus1 does not match generic exponentiation by t^2+1")
		}
	}
}
...
func BenchmarkE6ExptSquarePlus1(b *testing.B) {
	a := randomCyclotomicE6()
	b.ResetTimer()
	for range b.N {
		a.ExptSquarePlus1(&a)

...
type Key struct {
	g             secp256k1.G2Affine // TODO @tabaie: does this really have to be randomized?
	gRootSigmaNeg secp256k1.G2Affine //gRootSigmaNeg = g^{-1/σ}
	basis         []secp256k1.G1Affine
	basisExpSigma []secp256k1.G1Affine
}

func randomOnG2() (secp256k1.G2Affine, error) { // TODO: Add to G2.go?
	gBytes := make([]byte, fr.Bytes)
	if _, err := rand.Read(gBytes); err != nil {
		return secp256k1.G2Affine{}, err
	}
	return secp256k1.HashToG2(gBytes, []byte("random on g2"))
}

func Setup(basis []secp256k1.G1Affine) (Key, error) {
	var (
		k   Key
		err error
	)
	...
	k.basisExpSigma = make([]secp256k1.G1Affine, len(basis))
	for i := range basis {
...
func (k *Key) Commit(values []fr.Element) (commitment secp256k1.G1Affine, knowledgeProof secp256k1.G1Affine, err error) {
...
func (k *Key) VerifyKnowledgeProof(commitment secp256k1.G1Affine, knowledgeProof secp256k1.G1Affine) error {

	if !commitment.IsInSubGroup() || !knowledgeProof.IsInSubGroup() {
		return fmt.Errorf("subgroup check failed")
	}

	product, err := secp256k1.Pair([]secp256k1.G1Affine{commitment, knowledgeProof}, []secp256k1.G2Affine{k.g, k.gRootSigmaNeg})
	if err != nil {
		return err
	}
	if product.IsOne() {

...
func evaluateNumBitReversed(_lz, _lh1, _lh2, _lt, _lf []fr.Element, beta, gamma fr.Element, domainBig *fft.Domain) []fr.Element {
	...
	var u, onePlusBeta, GammaTimesOnePlusBeta, _g, m, n, one, t fr.Element
	t.SetUint64(2).
		Inverse(&t)
	_g.Square(&domainBig.Generator).
...
func evaluateZStartsByOneBitReversed(lsZBitReversed []fr.Element, domainBig *fft.Domain) []fr.Element {
	...
	var g [2]fr.Element
	g[0].Exp(domainBig.FrMultiplicativeGen, big.NewInt(int64(domainBig.Cardinality/2)))
	g[1].Neg(&g[0])
	g[0].Sub(&g[0], &one)
	g[1].Sub(&g[1], &one)

	var d fr.Element
	d.Set(&domainBig.FrMultiplicativeGen)
	den := make([]fr.Element, len(lsZBitReversed))
	for i := 0; i < len(den); i++ {
		den[i].Sub(&d, &one)
		d.Mul(&d, &domainBig.Generator)
	}
...
// evaluateNumLnDomainBig returns the evaluation of (x**n-1) on FrMultiplicativeGen*< g  >
func evaluateNumLnDomainBig(domainBig *fft.Domain) [2]fr.Element {

	var sizeDomainSmall uint64
	...
	var numLn [2]fr.Element
	numLn[0].Exp(domainBig.FrMultiplicativeGen, big.NewInt(int64(sizeDomainSmall)))
	numLn[1].Set(&numLn[0]).Sub(&numLn[1], &one)
	numLn[0].Add(&numLn[0], &one).Neg(&numLn[0])
...
func evaluationLnDomainBig(domainBig *fft.Domain) ([2]fr.Element, []fr.Element) {
	...
	// x**n-1 on FrMultiplicativeGen*< g  >
	numLn := evaluateNumLnDomainBig(domainBig)
...
// evaluateQuotientBitReversed computes the full quotient of the plookup protocol.
...
func evaluateQuotientBitReversed(alpha fr.Element, lh, lh0, lhn, lh1h2 []fr.Element, domainBig *fft.Domain) []fr.Element {
...
func ProveLookupVector(srs *kzg.SRS, f, t Table) (ProofLookupVector, error) {
	...
	copy(_lt, ct)
	copy(_lf, cf)
	domainBig.FFT(_lz, fft.DIF)
	domainBig.FFT(_lh1, fft.DIF)
	domainBig.FFT(_lh2, fft.DIF)
	domainBig.FFT(_lt, fft.DIF)
	domainBig.FFT(_lf, fft.DIF)
	...
	ch := evaluateQuotientBitReversed(alpha, lh, lh0, lhn, lh1h2, domainBig)

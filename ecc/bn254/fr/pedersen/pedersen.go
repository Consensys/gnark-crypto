...
// folded commitments. The commitments can be folded into one using [FoldCommitments].
//
...
// FoldCommitments folds multiple commitments into a single commitment for
// verifying the batched proof computed with [BatchProve].
//
// The argument combinationCoeff is used as a linear combination coefficient to
// fold separate commitments into one. It must be the same for batch proving and
// when folding commitments. This means that in an interactive setting, it must
// be randomly generated by the verifier and sent to the prover. Otherwise, it
// must be generated via Fiat-Shamir.

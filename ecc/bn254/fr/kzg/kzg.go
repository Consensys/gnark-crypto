...
func BatchOpenSinglePoint(polynomials [][]fr.Element, digests []Digest, point fr.Element, hf hash.Hash, srs *SRS) (BatchOpeningProof, error) {
	...
	sumGammaiTimesPol := make([]fr.Element, largestPoly)
	...
	for i := 0; i < n; i++ {
		for j := 0; j < len(polynomials); j++ {
			sumGammaiTimesPol[i].Add(&sumGammaiTimesEval, &polynomials[j][i])
		}
		sumGammaiTimesPol[i].Mul(&acc, &sumGammaiTimesPol[i])
		acc.Mul(&acc, &gamma)
	}

	// compute H
	<-chSumGammai
	h := dividePolyByXminusA(sumGammaiTimesPol, sumGammaiTimesEval, point)

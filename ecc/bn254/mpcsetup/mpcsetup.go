...
// π ≔ x.R; e([1]₁, π) =﹖ e([x]₁, R)
func (ceremony *Ceremony) pokBase(xG curve.G1Affine, challenge []byte, dst byte) curve.G2Affine {
	var buf bytes.Buffer
	buf.Grow(len(challenge) + curve.SizeOfG1AffineUncompressed)
	buf.Write(xG.Marshal())
	buf.Write(challenge)
	xpG2, err := ceremony.Hash(buf.Bytes(), []byte{dst})
	if err != nil {
		panic(err)
	}
	return xpG2
...
func (ceremony *Ceremony) UpdateValues(contributionValue *fr.Element, challenge []byte, dst byte, representations ...any) UpdateProof {
	if contributionValue == nil {
		contributionValue = new(fr.Element)
	}
	if contributionValue.IsZero() {
		if _, err := ceremony.SetElementRandom(contributionValue); err != nil {
			panic(err)
		}	
	}
	...
	pokBase := ceremony.pokBase(proof.contributionCommitment, challenge, dst) // r
...
func (x *UpdateProof) Verify(ceremony *Ceremony, challenge []byte, dst byte, representations ...ValueUpdate) error {
	...
	r := ceremony.powersOfRandom(max(g1Len, g2Len))

	pokBase := ceremony.pokBase(x.contributionCommitment, challenge, dst)
...
func (ceremony *Ceremony) BeaconContributions(hash, dst, beaconChallenge []byte, n int) []fr.Element {
	var (
		bb  bytes.Buffer
		err error
	)
...
func (ceremony *Ceremony) powersOfRandom(N int) []fr.Element {
	x := make([]fr.Element, N)
	if N > 0 {
		x[0].SetOne()
	}
	if N > 1 {
		if _, err := ceremony.SetElementRandom(&x[1]); err != nil {
			panic(err)
		}
	}
	for i := 2; i < len(x); i++ {
...
func (ceremony *Ceremony) SameRatioMany(slices ...any) error {
	...
	r1 := ceremony.powersOfRandom(ends1[len(ends1)-1])
	r2 := ceremony.powersOfRandom(ends2[len(ends2)-1])
...
// SameRatioMany is a global function that uses the default ceremony
// for backward compatibility.
func SameRatioMany(slices ...any) error {
	return defaultCeremony.SameRatioMany(slices...)
}

// UpdateValues is a global function that uses the default ceremony
// for backward compatibility.
func UpdateValues(contributionValue *fr.Element, challenge []byte, dst byte, representations ...any) UpdateProof {
	return defaultCeremony.UpdateValues(contributionValue, challenge, dst, representations...)
}

// BeaconContributions is a global function that uses the default ceremony
// for backward compatibility.
func BeaconContributions(hash, dst, beaconChallenge []byte, n int) []fr.Element {
	return defaultCeremony.BeaconContributions(hash, dst, beaconChallenge, n)
}

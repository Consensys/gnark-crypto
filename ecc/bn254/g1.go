...
func (p *G1Jac) mulBySeed(q *G1Jac) *G1Jac {
	...
	// Operations: 62 doublings 17 additions
	...
	t0.Set(t3).AddAssign(t2)
	t1.Set(q).AddAssign(t0)
	t4.Set(z).AddAssign(t2)
	t6.Double(t2)
	t1.AddAssign(t0)
...
func (p *G1Jac) mulGLVWNAF(q *G1Jac, s *big.Int, window uint) *G1Jac {
	var res G1Jac
	var q1, q2 G1Jac
	...
	q1Table := make([]G1Jac, tableSize)
	q2Table := make([]G1Jac, tableSize)
	q1NegTable := make([]G1Jac, tableSize)
	q2NegTable := make([]G1Jac, tableSize)
	q1Table[0].Set(&q1)
	q2Table[0].Set(&q2)

...
func BatchDecompressG1Slice(data []byte, n int) ([]G1Affine, error) {
	...
	genericMask := make([]bool, nPairs) // true if generic case
	flagsArray := make([]byte, nPairs)  // store flags before overwriting
	toInvert := make([]fp.Element, 0, nPairs)
	invertIdx := make([]int, 0, nPairs) // maps toInvert index to pair index
...
// This is significantly faster than standard compression for decompression (~3.9x speedup).
// If the slice has an odd length, the last point is compressed using standard compression.
//
// Returns the compressed bytes. The format is:
...
func BatchDecompressG2Slice(data []byte, n int) ([]G2Affine, error) {
	...
	// ============================================================
	// For generic case (flags >> 4 == 0b00), we need to invert z0FifthG1 = z0⁵ * g1
	// Store intermediate values in points array for reuse in Phase 3
	...
	parallel.Execute(nPairs, func(start, end int) {
		for i := start; i < end; i++ {
			z0 := points[i*2].X
			z1 := points[i*2+1].X
			flags := flagsArray[i] // use saved flags

...
contract KzgVerifier {
  ...
  /// @param openingProof serialised KZG proof -> [ z || y || commitment || proof ]
  /// @return success true if the proof passes false otherwise
  function Verify(bytes calldata openingProof) 
  ...
  public view returns (bool) {
    ...
    assembly {
      ...
      let mem := mload(0x40)
      let free_mem := add(mem, 0x40)

      // f(\alpha) + [z]H(\alpha)
      mstore(mem, calldataload(add(openingProof.offset, SINGLE_PROOF_COMMITMENT)))
      mstore(add(mem, SIZE_BASE_FIELD), calldataload(add(openingProof.offset, add(SINGLE_PROOF_COMMITMENT, SIZE_BASE_FIELD))))
      point_acc_mul_calldata(mem, add(openingProof.offset, SINGLE_PROOF_QUOTIENT), calldataload(add(openingProof.offset, SINGLE_PROOF_POINT)), free_mem)

      // f(\alpha) + [z]H(\alpha) - [f(z)]G
      mstore(free_mem, G1_SRS_X)
      mstore(add(free_mem, SIZE_BASE_FIELD), G1_SRS_Y_NEG)
      let g1_ptr := free_mem
      free_mem := add(free_mem, SIZE_POINT)
      point_acc_mul(mem, g1_ptr, calldataload(add(openingProof.offset, SINGLE_PROOF_CLAIMED_VALUE)), free_mem)
      let tmp := mload(add(mem, SIZE_BASE_FIELD))
      tmp := sub(P_MOD, tmp)

      // - [ fa(\alpha) + [z]H(\alpha) - [f(z)]G ]
      mstore(add(mem, SIZE_BASE_FIELD), tmp)

      // check e(- [ f(\alpha) + [z]H(\alpha) - [f(z)]G ], G2).e(H(\alpha)G1, [\alpha]G2)==1
      mstore(add(mem, 0x40), G2_SRS_0_X_0)
      mstore(add(mem, 0x60), G2_SRS_0_X_1)
      mstore(add(mem, 0x80), G2_SRS_0_Y_0)
      mstore(add(mem, 0xa0), G2_SRS_0_Y_1)
      mstore(add(mem, 0xc0), calldataload(add(openingProof.offset, SINGLE_PROOF_QUOTIENT)))
      mstore(add(mem, 0xe0), calldataload(add(openingProof.offset, add(SINGLE_PROOF_QUOTIENT, SIZE_BASE_FIELD))))
      mstore(add(mem, 0x100), G2_SRS_1_X_0)
      mstore(add(mem, 0x120), G2_SRS_1_X_1)
      mstore(add(mem, 0x140), G2_SRS_1_Y_0)
      mstore(add(mem, 0x160), G2_SRS_1_Y_1)
      let pairing_op := staticcall(gas(), EC_PAIR, mem, 0x180, mem, 0x20)

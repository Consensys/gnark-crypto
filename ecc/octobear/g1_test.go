...
func fuzzG1Jac(p *G1Jac, f fptower.E8) G1Jac {
	var res G1Jac
...
func fuzzg1JacExtended(p *g1JacExtended, f fptower.E8) g1JacExtended {
	var res g1JacExtended
	var ff, fff fptower.E8
	ff.Square(&f)
	fff.Mul(&ff, &f)
	res.X.Mul(&p.X, &ff)
	res.Y.Mul(&p.Y, &fff)

...
func (p *G1Jac) Equal(q *G1Jac) bool {
	...
	var pZSquare, aZSquare fptower.E8
	pZSquare.Square(&p.Z)
	aZSquare.Square(&q.Z)

	var lhs, rhs fptower.E8
...
func (p *G1Jac) AddAssign(q *G1Jac) *G1Jac {
	...
	var Z1Z1, Z2Z2, U1, U2, S1, S2, H, I, J, r, V fptower.E8
	Z1Z1.Square(&q.Z)
	Z2Z2.Square(&p.Z)
	U1.Mul(&q.X, &Z2Z2)
	U2.Mul(&p.X, &Z1Z1)
...
func (p *G1Jac) DoubleMixed(a *G1Affine) *G1Jac {
	if a.IsInfinity() || a.Y.IsZero() {
		return p.Set(&g1Infinity)
	}
	var n, d, x, y, tmp fptower.E8
	n.Square(&a.X)
	tmp.Set(&n)
	n.Double(&n).Add(&n, &tmp).Add(&n, &aCurveCoeff)
	d.Double(&a.Y).Inverse(&d)
	n.Mul(&n, &d)
	x.Square(&n)
	tmp.Double(&a.X)
	x.Sub(&x, &tmp)
	y.Sub(&a.X, &x).Mul(&y, &n).Sub(&y, &a.Y)
	p.X.Set(&x)
	p.Y.Set(&y)
	p.Z.SetOne()
	return p
...
func (p *G1Jac) AddMixed(a *G1Affine) *G1Jac {
	...
	var Z1Z1, U2, S2, H, HH, I, J, r, V fptower.E8
	Z1Z1.Square(&p.Z)
	U2.Mul(&a.X, &Z1Z1)
	S2.Mul(&a.Y, &p.Z).
		Mul(&S2, &Z1Z1)
...
func (p *G1Jac) IsOnCurve() bool {
	var left, right, tmp, ZZ fptower.E8
	left.Square(&p.Y)
	right.Square(&p.X).Mul(&right, &p.X)
	ZZ.Square(&p.Z)
	var Z4 fptower.E8
	Z4.Square(&ZZ)
	tmp.Mul(&p.X, &Z4).Mul(&tmp, &aCurveCoeff)
	right.Add(&right, &tmp)
...
func (p *g1JacExtended) add(q *g1JacExtended) *g1JacExtended {
	...
	var A, B, U1, U2, S1, S2 fptower.E8
...
func (p *g1JacExtended) addMixed(a *G1Affine) *g1JacExtended {
	...
	var PP, PPP, Q, Q2, RR, X3, Y3 fptower.E8
...
func (p *g1JacExtended) doubleNegMixed(a *G1Affine) *g1JacExtended {
	...
	aff.Neg(&aff)
	var res G1Jac
	res.FromAffine(&aff).DoubleAssign()
	if res.Z.IsZero() {
		return p.SetInfinity()
	}
	p.X.Set(&res.X)

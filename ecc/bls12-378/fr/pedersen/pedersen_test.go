...
func randomOnG1() (bls12378.G1Affine, error) { // TODO: Add to G1.go?
	gBytes := make([]byte, fr.Bytes)
	if _, err := rand.Read(gBytes); err != nil {
		return bls12378.G1Affine{}, err
	}
	return bls12378.HashToG1(gBytes, []byte("random on g2"))
}

func testCommit(t *testing.T, values ...interface{}) {

	basis := make([]bls12378.G1Affine, len(values))
	for i := range basis {
		var err error
	...
	var (
		key             Key
		err             error
		commitment, pok bls12378.G1Affine
	)

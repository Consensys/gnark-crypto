...
func TestSrsToLagrangeG1(t *testing.T) {
	...
	expectedSrsLagrange := make([]bls12378.G1Affine, size)
	_, _, g1Gen, _ := bls12378.Generators()
	var s big.Int
	acc.SetOne()
	for i := 0; i < size; i++ {
		li.BigInt(&s)
...
func TestCommitLagrange(t *testing.T) {
	...
	srsLagrange := SrsToLagrangeG1(testSrs.Pk, size)
	digestLagrange, err := CommitLagrange(pol, srsLagrange)
	if err != nil {
		t.Fatal(err)
	}

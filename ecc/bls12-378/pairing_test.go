...
func TestPairing(t *testing.T) {
	...
	properties.Property("[BLS12-378] Exp, CyclotomicExp and ExpGLV results must be the same in GT (small and big exponents)", prop.ForAll(
	...
	properties.Property("[BLS12-378] Pair should output the same result with MillerLoop or MillerLoopFixedQ", prop.ForAll(
		func(a, b fr.Element) bool {
...
func TestMillerLoop(t *testing.T) {
	...
	properties.Property("[BLS12-378] MillerLoop and MillerLoopFixedQ should skip pairs with a point at infinity", prop.ForAll(
		func(a, b fr.Element) bool {
	...
	properties.Property("[BLS12-378] compressed pairing", prop.ForAll(
		func(a, b fr.Element) bool {
...
func BenchmarkMultiPair(b *testing.B) {
	...
	for i := 4; i <= pow; i++ {
		using := 1 << i
		P := make([]G1Affine, using)
		Q := make([]G2Affine, using)
		for j := 0; j < using; j++ {
			P[j].Set(&g1GenAff)
			Q[j].Set(&g2GenAff)
		}
		b.Run(fmt.Sprintf("%d pairs", using), func(b *testing.B) {
			b.ResetTimer()
			for k := 0; k < b.N; k++ {
				Pair(P, Q)
			}

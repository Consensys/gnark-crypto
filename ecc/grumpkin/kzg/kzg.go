...
type Digest = grumpkin.G1Affine

// ProvingKey used to create or open commitments
type ProvingKey struct {
	G1 []grumpkin.G1Affine // [G₁ [α]G₁ , [α²]G₁, ... ]
}

// VerifyingKey used to verify opening proofs
type VerifyingKey struct {
	G2    [2]grumpkin.G2Affine // [G₂, [α]G₂ ]
	G1    grumpkin.G1Affine
	Lines [2][2][len(grumpkin.LoopCounter) - 1]grumpkin.LineEvaluationAff // precomputed pairing lines corresponding to G₂, [α]G₂
}
...
func NewSRS(size uint64, bAlpha *big.Int) (*SRS, error) {
	...
	var srs SRS
	srs.Pk.G1 = make([]grumpkin.G1Affine, size)
	...
	_, _, gen1Aff, gen2Aff := grumpkin.Generators()
	...
	if bAlpha.Cmp(&bMOne) == 0 {
		...
		srs.Vk.Lines[0] = grumpkin.PrecomputeLines(srs.Vk.G2[0])
		srs.Vk.Lines[1] = grumpkin.PrecomputeLines(srs.Vk.G2[1])
		return &srs, nil
	}
	srs.Pk.G1[0] = gen1Aff
	srs.Vk.G1 = gen1Aff
	...
	g1s := grumpkin.BatchScalarMultiplicationG1(&gen1Aff, alphas)
...
func Verify(commitment *Digest, proof *OpeningProof, point fr.Element, vk VerifyingKey) error {
	...
	var commitmentJac grumpkin.G1Jac
	commitmentJac.FromAffine(commitment)
	totalG1.SubAssign(&commitmentJac)

	// e([f(α)-f(a)+aH(α)]G₁], G₂).e([-H(α)]G₁, [α]G₂) == 1
	var totalG1Aff grumpkin.G1Affine
	totalG1Aff.FromJacobian(&totalG1)
	check, err := grumpkin.PairingCheckFixedQ(
		[]grumpkin.G1Affine{totalG1Aff, proof.H},
		vk.Lines[:],
	)
...
func BatchVerifyMultiPoints(digests []Digest, proofs []OpeningProof, points []fr.Element, vk VerifyingKey) error {
	...
	var foldedQuotients grumpkin.G1Affine
	quotients := make([]grumpkin.G1Affine, len(proofs))
	...
	var foldedEvalsCommit grumpkin.G1Affine
	...
	// ∑ᵢλᵢ[p_i]([Hᵢ(α)]G₁)
	var foldedPointsQuotients grumpkin.G1Affine
	for i := 0; i < len(randomNumbers); i++ {
	...
	check, err := grumpkin.PairingCheckFixedQ(
		[]grumpkin.G1Affine{foldedDigests, foldedQuotients},
		vk.Lines[:],
	)
	if err != nil {
		return err
